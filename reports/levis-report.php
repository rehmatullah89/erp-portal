		  ...
		  <table border="0" cellpadding="0" cellspacing="0" width="100%">
			<tr valign="top">
			  <td width="100%">
			    ...
			    <form name="frmSearch" id="frmSearch" method="post" action="<?= SITE_URL."reports/export-levis-report.php" ?>" class="frmOutline" onsubmit="checkDoubleSubmission( );">
				<h2>Levis Reports</h2>
...
<?
	...
	$sSQL = "SELECT id, CONCAT(`code`,' - ', `vendor`) as _Vendor FROM tbl_vendors WHERE id IN ({$_SESSION['Vendors']}) AND id IN ($sVendors) AND parent_id='0' AND sourcing='Y' ORDER BY vendor";
	$objDb->query($sSQL);
	...
	{
		$sKey   = $objDb->getField($i, "id");
		$sValue = $objDb->getField($i, "_Vendor");
?>
	  	        		<option value="<?= $sKey ?>"><?= $sValue ?></option>
<?
	}
...
<?
	...
	if ($_SESSION["UserType"] == "LEVIS")
		$sAuditorsList = getList("tbl_users", "id", "name", "status='A' AND auditor='Y' AND user_type='LEVIS'");
				  ...
				  <tr>
					...
					<td>
					  <select name="ReportType" style="width:300px;">
						<option value="IR">Inspection Report</option>
						<option value="MR">Measurement Report</option>
					  </select>
					</td>
				  </tr> 
				</table>
