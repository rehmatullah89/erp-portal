	...
	***********************************************************************************************
	...
	if ($FromDate != "" && $ToDate != "" && $ReportType == 'QS')
	{
        ...
        $sBorderStyle = array('alignment' => array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_LEFT),
            ...
            $sReportTypes  = implode(",",array_intersect(explode(",", $sReportTypes1),array(14,34,47)));;
            
            $sAuditStages1 = getDbValue("audit_stages", "tbl_users", "id='{$_SESSION['UserId']}'");            
            $sAuditStages2 = getDbValue("stages", "tbl_reports", "id IN (14,34,47)");
...
/*            $sPos = "";
        ...
        if($ReportType == 'QS')
        {
                ...
                $iMeasurementDefectsList = getList("tbl_qa_report_defects qrd, tbl_qa_reports qa", "qa.id", "COUNT(1)", "qa.id=qrd.audit_id AND qa.report_id IN (14,34,47) AND FIND_IN_SET(qrd.code_id, '$sMeasurementCodes') GROUP BY qa.id");
                $iWorkmanshipDefectsList = getList("tbl_qa_report_defects qrd, tbl_qa_reports qa", "qa.id", "COUNT(1)", "qa.id=qrd.audit_id AND qa.report_id IN (14,34,47) AND FIND_IN_SET(qrd.code_id, '$sWorkmanshipCodes') GROUP BY qa.id");
                $iMaterialDefectsList    = getList("tbl_qa_report_defects qrd, tbl_qa_reports qa", "qa.id", "COUNT(1)", "qa.id=qrd.audit_id AND qa.report_id IN (14,34,47) AND FIND_IN_SET(qrd.code_id, '$sMaterialCodes') GROUP BY qa.id");
