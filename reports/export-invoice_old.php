...
*/
		...
		$sSQL = ("SELECT id, currency FROM tbl_po WHERE FIND_IN_SET(brand_id, '".@implode(",", $Brand)."')");
		...
		$sSQL = "SELECT report_id, group_id, brand_id, po_id, additional_pos, audit_code, colors, total_gmts, audit_date, audit_mode, start_time, end_time, commission_type, style_id, ship_qty,
		...
		for ($i = 0; $i < $iCount; $i ++)
		{
			...
			$sCommissionType = $objDb->getField($i, "commission_type");
                        $iReportType     = $objDb->getField($i, "report_id");
			...
			foreach ($iAuditPos as $iPo)
			{
                                ...
                                if($BilledTo == "Social Fashion Company GmbH, Thebäerstr.17, 50823 Cologne, GDR")
                                {
                                        /*$fFobValue = 0;
                                        $iOrderQty = 0;
                                        ...
                                        {
                                            $OrderQty  = getDbValue("order_qty", "tbl_po_colors", "color LIKE '$iColor' AND po_id = '$iPo' AND style_id='$iStyle'"); 

                                            if($OrderQty != "")
                                                $iOrderQty += $OrderQty; 
                                            
                                            if ($Quantity == "O")
                                                $fFobValue += getDbValue("SUM(price * order_qty)", "tbl_po_colors", "color LIKE '$iColor' AND po_id='$iPo' AND style_id='$iStyle'");

                                            else
                                                $fFobValue += ($iShipQty * getDbValue("MAX(price)", "tbl_po_colors", "color LIKE '$iColor' AND po_id='$iPo' AND style_id='$iStyle'"));
                                            
                                        }*/
                                    $iColors = explode(",", $sColors);
                                    foreach ($iColors as $iColor)
                                    {
                                        $price = getDbValue("price", "tbl_po_colors", "color LIKE '$iColor' AND po_id='$iPo' AND style_id='$iStyle'");
                                    }
                                    
                                    $iOrderQty = $iShipmentQty;
                                    $fFobValue = $iOrderQty*$price;
                                ...
                                if ($sCommissionType == "V")
                                    ...
                                    $sHtml .= '<tr>
                                                             ...
                                                             <td width="80">'.$sCurrencySymbol.formatNumber($fFobValue).'</td>
                                                             <td width="80">'.$sCurrencySymbol.formatNumber($fCommission).'</td>
                                                       </tr>';
		...
		$sHtml .= '<tr>
					 <td colspan="6" align="right">Total</td>
                                         ...
                                         <td colspan="2">&nbsp;</td>
                                         <td>'.$sCurrencySymbol.formatNumber($fFobTotal).'</td>
					 <td>'.$sCurrencySymbol.formatNumber($fTotal).'</td>
				   </tr>';
