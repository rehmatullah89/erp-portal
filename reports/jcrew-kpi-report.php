...
<!--  Body Section Starts Here  -->
	    <div id="Body">
		  <table border="0" cellpadding="0" cellspacing="0" width="100%">
			<tr valign="top">
			  <td width="100%">
			    <h1>j.crew kpi report</h1>

			    <form name="frmSearch" id="frmSearch" method="post" action="<?= SITE_URL.'reports/export-jcrew-kpi-report.php' ?>" class="frmOutline" onsubmit="checkDoubleSubmission( );">
				<h2>J.Crew KPI Report</h2>
...
<?
	...
	$sAuditStagesList = getList("tbl_audit_stages", "code", "stage", "code IN ('". implode("','", explode(",", $sJcrewStages))."') AND code='F'");
...
<?
        ...
        foreach($iVendors as $iVendor)
        {
            if($iVendor != 0)
            {
                $sUserFactories = getList("tbl_users", "id", "id", "FIND_IN_SET($iVendor, vendors) AND auditor='Y' AND status='A' AND email NOT LIKE '%@3-tree.com%' AND email NOT LIKE '%@jcrew.com' AND user_type='JCREW'");
                $sUsers[] = implode(",", $sUserFactories);
            }
        }
        ...
        if(@in_array($_SESSION["UserType"], array("JCREW")))
            $sAuditorsList  = getList("tbl_users", "id", "name", "auditor='Y' AND status='A' AND email NOT LIKE '%@3-tree.com%' AND email NOT LIKE '%@jcrew.com' AND user_type='JCREW' AND id IN ($sUsers)");        
