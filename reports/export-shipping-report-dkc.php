<?
	...
	***********************************************************************************************
	...
	$ShFromDate  = IO::strValue("FromDate");
	$ShToDate    = IO::strValue("ToDate");
 ...
 *              $sSQL = "SELECT po.order_no, pc.color, pc.order_qty, po.order_status, po.vendor_id, po.shipping_dates, po.destinations,
                        (SELECT quantity FROM tbl_pre_shipment_quantities WHERE po.id=po_id AND pc.id=color_id Limit 1) AS _ShipQty,
	                (SELECT style FROM tbl_styles WHERE id IN (po.styles) ORDER BY id LIMIT 1) AS _Style,
	                (SELECT sub_season_id FROM tbl_styles WHERE id IN (po.styles) ORDER BY id LIMIT 1) AS _Season,
 ...
 */
	$sSQL = "SELECT po.order_no, po.order_status, po.vendor_id, po.shipping_dates, po.destinations,
                        (SELECT GROUP_CONCAT(DISTINCT(color) SEPARATOR ', ') FROM tbl_po_colors WHERE po.id=po_id) AS _Color,
                        (SELECT GROUP_CONCAT(price SEPARATOR ', ') FROM tbl_po_colors WHERE po.id=po_id) AS _Price,
                        (SELECT SUM(price*order_qty) FROM tbl_po_colors WHERE po.id=po_id) AS _PriceValue,
                        (SELECT SUM(order_qty) FROM tbl_po_colors WHERE po.id=po_id) AS _OrderQty,                        
                        (SELECT SUM(quantity) FROM tbl_pre_shipment_quantities WHERE po.id=po_id) AS _ShipQty,
                        (SELECT style FROM tbl_styles WHERE id IN (po.styles) ORDER BY id LIMIT 1) AS _Style,
	                (SELECT sub_season_id FROM tbl_styles WHERE id IN (po.styles) ORDER BY id LIMIT 1) AS _Season,
                        (SELECT GROUP_CONCAT(DISTINCT(tod.terms) SEPARATOR '; ') FROM tbl_pre_shipment_detail psd, tbl_terms_of_delivery tod WHERE tod.id=psd.terms_of_delivery_id AND psd.po_id = po.id) AS _ShipMode,
                        (SELECT shipping_date FROM tbl_pre_shipment_detail WHERE po_id = po.id LIMIT 1) AS _ShipDate,
	...
	@fwrite($hFile, ('"Order No","Color","Price","Value","Vendor","Style No","Season","PO ETD","Handover to Forwarder","Difference (Days)","Destination","Order Qty","Ship Qty","Short Ship","Vsl Sailing Date","Shipping Mode"'."\n"));
	...
	for ($i = 0; $i < $iCount; $i ++)
	{
		...
		$sLine = ('"'.
		          ($objDb->getField($i, 'order_no').' '.$objDb->getField($i, 'order_status')).'","'.
                          ($objDb->getField($i, '_Color')).'","'.  
                          ($objDb->getField($i, '_Price')).'","'.
                          ($objDb->getField($i, '_PriceValue')).'","'.
				  ...
				  $sDestinations.'","'.
				  formatNumber($objDb->getField($i, '_OrderQty'), false).'","'.
				  formatNumber($objDb->getField($i, '_ShipQty'), false).'","'.
                                  formatNumber(($objDb->getField($i, '_ShipQty') - $objDb->getField($i, '_OrderQty')), false).'","'.
