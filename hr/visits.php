...
?>
  <script type="text/javascript" src="scripts/hr/visits.js"></script>
</head>
...
<?
	...
	{
?>
			    <form name="frmData" id="frmData" method="post" action="hr/save-visit.php" class="frmOutline" onsubmit="$('BtnSave').disabled=true;">
...
?>
				  ...
				  <tr>
				    <td>Visit Type<span class="mandatory">*</span></td>
				    <td align="center">:</td>

				    <td>
					  <select name="VisitType" onchange="resetLocations(this.value);">
					    <option value=""></option>
					    <option value="Lunch"<?= (($VisitType == "Lunch") ? " selected" : "") ?>>Lunch</option>
					    <option value="Visit"<?= (($VisitType == "Visit") ? " selected" : "") ?>>Visit</option>
					  </select>
				    </td>
				  </tr>
				</table>
...
?>
				    <tr>
					  <td width="80">Location # <?= $i ?><?= (($i == 1) ? '<span class="mandatory">*</span>' : '') ?></td>
					  <td width="20" align="center">:</td>
...
<?
			...
			{
?>
			              <option value="<?= $sKey ?>"<?= (($sKey == IO::intValue("Location".$i)) ? " selected" : "") ?>><?= $sValue ?></option>
<?
			}
?>
		                </select>
...
<?
	...
	if ($VisitType != "")
		$sConditions .= " AND type='VisitType' ";

	@list($iTotalRecords, $iPageCount, $iStart) = getPagingInfo("tbl_user_visits", $sConditions, $iPageSize, $PageId);


	$sSQL = "SELECT * FROM tbl_user_visits $sConditions ORDER BY id DESC LIMIT $iStart, $iPageSize";
	$objDb->query($sSQL);
...
?>
				  <table border="1" bordercolor="#ffffff" cellpadding="5" cellspacing="0" width="100%">
				    <tr class="headerRow">
				      ...
				      <td width="30%">Visit Type / Visit Locations</td>
...
<?
		...
		else
		{
			...
			$iCount2    = count($iLocations);

			for ($j = 1; $j < $iCount2; $j ++)
				$sDetail .= ((($j > 1) ? ', ' : '').$sLocationsList[$iLocations[$j]]);
		}
?>
			      ...
			      <table border="1" bordercolor="#ffffff" cellpadding="5" cellspacing="0" width="100%">
				    <tr class="<?= $sClass[($i % 2)] ?>">
				      ...
				      <td width="10%"><span id="TimeOut<?= $iId ?>"><?= formatTime($sTimeOut) ?></span></td>
				      <td width="10%"><span id="TimeIn<?= $iId ?>"><?= formatTime($sTimeIn) ?></span></td>
				      <td width="30%"><span id="Detail<?= $iId ?>"><?= $sDetail ?></span></td>
...
<?
		...
		{
?>
				        <a href="hr/delete-visit.php?Id=<?= $iId ?>" onclick="return confirm('Are you SURE, You want to Delete this Visit Record?');"><img src="images/icons/delete.gif" width="16" height="16" alt="Delete" title="Delete" /></a>
<?
		}
?>
				      </td>
...
?>
					    ...
					    <tr>
						  ...
						  <td>
						    <select name="VisitType" onchange="resetVisitLocations(this.value, <?= $iId ?>);">
							  <option value="Lunch"<?= (($sType == "Lunch") ? " selected" : "") ?>>Lunch</option>
							  <option value="Visit"<?= (($sType == "Visit") ? " selected" : "") ?>>Visit</option>
						    </select>
						  </td>
					    </tr>
					  </table>

					  <div id="VisitLocations<?= $iId ?>" style="display:<?= (($sType == 'Visit') ? 'display' : 'none') ?>;">
					    <table border="0" cellpadding="3" cellspacing="0" width="100%">
<?
		for ($j = 1; $j <= 8; $j ++)
		{
?>
						  <tr>
						    <td width="80">Location # <?= $j ?><?= (($j == 1) ? '<span class="mandatory">*</span>' : '') ?></td>
						    <td width="20" align="center">:</td>

						    <td>
							  <select name="Location<?= $j ?>">
							    <option value=""></option>
<?
			foreach ($sLocationsList as $sKey => $sValue)
			{
?>
			              	    <option value="<?= $sKey ?>"<?= (($sKey == $iLocations[$j]) ? " selected" : "") ?>><?= $sValue ?></option>
<?
			}
?>
							  </select>
...
?>
			      <table border="1" bordercolor="#ffffff" cellpadding="5" cellspacing="0" width="100%">
				    <tr>
				      <td class="noRecord">No Employee Visit Record Found!</td>
				    </tr>
				  </table>
<?
	}
...
<?
	showPaging($PageId, $iPageCount, $iCount, $iStart, $iTotalRecords, "&Employee={$Employee}&FromDate={$FromDate}&ToDate={$ToDate}&VisitType={$VisitType}");
?>
