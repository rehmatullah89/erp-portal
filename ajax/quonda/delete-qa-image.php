<?
	...
	***********************************************************************************************
	...
	if ($ImageId > 0)
	{
		$sSQL  = "DELETE FROM tbl_qa_report_images WHERE id='$ImageId' AND audit_id='$AuditId'";
		$objDb->execute($sSQL);
	}

	if ($File != "" && (@unlink("../".$sBaseDir.QUONDA_PICS_DIR.$sYear."/".$sMonth."/".$sDay."/".$File) || @unlink("../".$sBaseDir.SPECS_SHEETS_DIR.$sYear."/".$sMonth."/".$sDay."/".$File)))
	{
