...
<?
	...
	$sTermsOfDelivery     = $objDb->getField(0, "_TermsOfDelivery");
	$sModeOfTransport     = $objDb->getField(0, "mode_of_transport");
	$sCartons             = $objDb->getField(0, "cartons");
	$sHandoverToForwarder = formatDate($objDb->getField(0, "handover_to_forwarder"));
	$sShippingDate        = formatDate($objDb->getField(0, "shipping_date"));
	$sArrivalDate         = formatDate($objDb->getField(0, "arrival_date"));
	$sInvoicePackingList  = $objDb->getField(0, "invoice_packing_list");
	$sInvoiceNo           = $objDb->getField(0, "invoice_no");
	$sLadingAirwayBill    = $objDb->getField(0, "lading_airway_bill");
...
<?
	...
	$sSQL = "SELECT style_id FROM tbl_po_colors WHERE po_id='$iPoId'";
	$objDb2->query($sSQL);
	...
	$sSQL = "SELECT id, size FROM tbl_sizes WHERE id IN (SELECT DISTINCT(size_id) FROM tbl_po_quantities WHERE po_id='$iPoId') ORDER BY id";
	$objDb2->query($sSQL);
...
<?
		for ($j = 0; $j < $iPoSizesCount;)
		{
			for ($k = 0; $k < 15; $k ++)
			{
				if ($j < $iPoSizesCount)
				{
					$sSQL = "SELECT quantity FROM tbl_pre_shipment_quantities WHERE po_id='$iPoId' AND ship_id='$Id' AND color_id='$iColorId' AND size_id='{$sPoSizes[$j][0]}'";
					$objDb3->query($sSQL);
