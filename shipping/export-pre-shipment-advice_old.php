<?
	...
	***********************************************************************************************
	...
	@fwrite($hFile, ('"Order No","Vendor","Style No","Season","PO ETD","Handover to Forwarder","Difference (Days)","Destination","Order Qty","Ship Qty"'."\n"));
	...
	for ($i = 0; $i < $iCount; $i ++)
	{
		...
		$sLine = ('"'.
				  ...
				  $sDestinations.'","'.
				  formatNumber($objDb->getField($i, 'po.quantity'), false).'","'.
				  formatNumber($objDb->getField($i, 'psa.quantity'), false).'"'.
				"\n");
