...
?>
    <script type="text/javascript" src="scripts/crc/tnc-audit-images.js"></script>
</head>
...
<?
	$sAuditDate = getDbValue("audit_date", "tbl_tnc_audits", "id='$AuditId'");
	...
	$sSQL = "SELECT id, title, point_id, picture FROM tbl_tnc_audit_pictures WHERE audit_id='$AuditId'";
	$objDb->query($sSQL);
...
<?
					...
					{
?>
					    <a id="Delete<?= $i ?>" href="crc/delete-tnc-audit-image.php?File=<?= $sPicture ?>&ImageId=<?= $iImageId ?>&AuditId=<?= $AuditId ?>&AuditDate=<?= $sAuditDate ?>&Referer=<?= urlencode($Referer) ?>" onclick="return confirm('Are you SURE, You want to Delete this Image?');"><img src="images/icons/delete.gif" width="16" height="16" alt="Delete" title="Delete" /></a>
<?
					}
?>
					  </div>
...
?>
					  <script type="text/javascript">
						<!--
							var objEditor<?= $i ?> = new Ajax.InPlaceEditor('Pic<?= $i ?>', 'ajax/crc/update-tnc-audit-image.php', { cancelControl:'button', okText:' Ok ', cancelText:'Cancel', clickToEditText:'Click to Edit Title', highlightcolor:'#f1edcd', highlightendcolor:'#ffffff', callback:function(form, value) { return 'Id=<?= $iImageId ?>&Title=' + encodeURIComponent(value); }, onComplete:function( ) { }, onEnterEditMode:function(form, value) { $('Pic<?= $i ?>').focus( ); } });
						-->
					  </script>
<?
					}
...
?>
                ...
                <form name="frmData" id="frmData" method="post" action="crc/save-tnc-audit-images.php" enctype="multipart/form-data" class="frmOutline" onsubmit="$('BtnSave').disabled=true;">
...
?>
				<table border="0" cellpadding="2" cellspacing="0" width="280px">
				    ...
				    <td>
				      <select name="Section" id="Section" onchange="getCategories(this.value, 'Category')">
				        <option value=""></option>
<?
...
?>
				  ...
				  <tr>
					...
					<td>
					  <select name="Category" id="Category" onchange="getPoints(this.value, 'Point')">
						<option value=""></option>
<?
		if ($Section > 0)
		{
