	...
	***********************************************************************************************
	...
	for ($i = 1; $i <= 5; $i ++)
	{
		if ($_FILES["Image{$i}"]['name'] != "")
		{
                        ...
                        $iPictureId = getNextId("tbl_crc_audit_pictures");
			...
			if (@move_uploaded_file($_FILES["Image{$i}"]['tmp_name'], ($sTncDir.$sPicture)))
			{
				...
				$sSQL = "INSERT INTO tbl_crc_audit_pictures (id, audit_id, point_id, title, picture) VALUES ('$iPictureId', '$AuditId', '$PointId', '$Image', '$sPicture')";
                                $objDb->execute($sSQL);
	...
	redirect("crc-audit-images.php?AuditId={$AuditId}&Referer={$Referer}");
?>