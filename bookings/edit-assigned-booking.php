<?
	...
	***********************************************************************************************
	...
	$Id         = IO::intValue('Id');
        ...
        $iReport                = getDbValue("COUNT(1)", "tbl_qa_reports", "booking_id='$Id' AND audit_result != ''");
						  ...
						  <td>
				            <div>
				                <input type="hidden" id="Po" name="Po" value="<?= $iPoId ?>" />
                                                <input type="hidden" id="HohOrderNo" name="HohOrderNo" value="<?= htmlentities($HohOrderNo, ENT_QUOTES) ?>" />
								<input type="text" id="OrderNo" name="OrderNo" value="<?= htmlentities($HohOrderNo, ENT_QUOTES) ?>" size="20" maxlength="50" autocomplete="off" class="textbox" style="width:280px;" />
...
<?
	$sSizesList = getList("tbl_sampling_sizes", "id", "size", "id IN (SELECT DISTINCT(size_id) FROM tbl_hoh_order_details WHERE hoh_order_id='$iPoId')", "display_order");
        
	foreach ($sSizesList as $sKey => $sValue)
	{
?>
                                                        <option value="<?= $sKey ?>"<?= ((@in_array($sKey, explode(",", $sSizes))) ? ' selected' : '') ?>><?= $sValue ?></option>
<?
	}
?>
					        </select>
...
?>
						      <option value="0"<?= ((IO::strValue("SampleSize") != "" && $iTotalGmts == 0) ? " selected" : "") ?>>Custom</option>
						    </select>
						  </td>
					    </tr>
