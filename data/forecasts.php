...
?>
  <script type="text/javascript" src="scripts/data/forecasts.js"></script>
</head>
...
<?
	...
	{
?>
			    <form name="frmData" id="frmData" method="post" action="data/save-forecast.php" class="frmOutline" onsubmit="$('BtnSave').disabled=true;">
				<h2>Add Forecast</h2>
...
<?
	...
	@list($iTotalRecords, $iPageCount, $iStart) = getPagingInfo("tbl_forecasts", $sConditions, $iPageSize, $PageId);
...
<?
		...
		{
?>
				        <a href="data/delete-forecast.php?Id=<?= $iId ?>" onclick="return confirm('Are you SURE, You want to Delete this Forecast Entry?');"><img src="images/icons/delete.gif" width="16" height="16" alt="Delete" title="Delete" /></a>
<?
		}
?>
				      </td>
...
?>
			      <table border="1" bordercolor="#ffffff" cellpadding="5" cellspacing="0" width="100%">
				    <tr>
				      <td class="noRecord">No Forecast Record Found!</td>
				    </tr>
			      </table>
<?
	}
...
<?
	...
	$sSQL = "SELECT SUM(quantity) FROM tbl_forecasts $sConditions";
	$objDb->query($sSQL);
...
?>
				...
				<div class="buttonsBar" style="margin-top:4px;">
				  <span style="float:right; color:#ffffff; font-weight:bold; font-size:12px; padding:8px 10px 0px 0px;">Forecast Quantity: <?= formatNumber($iQuantity, false) ?></span>
<?
	if ($iQuantity > 0)
	{
?>

				  <input type="hidden" id="ExportUrl" name="ExportUrl" value="<?= (SITE_URL."data/export-forecasts.php?Month={$Month}&Year={$Year}&Region={$Region}&Category={$Category}&Vendor={$Vendor}&Brand={$Brand}") ?>" />
