<?
	...
	$iSamplingCategory = IO::intValue("SamplingCategory");	
        ...
        $sCheckDiscardedPoints = array();
	$sSizesList = getList("tbl_sampling_sizes", "id", "size", "", "id");
	$position = getDbValue("MAX(position)", "tbl_style_specs")+1;
	...
	while (($sRecord = @fgetcsv($hFile, 10000)) !== FALSE)
	{
		if ($sRecord[0] == "PC5 Code" && $sRecord[1] == "Product Name")
		{
			...
			$iStyle = (int)getDbValue("id", "tbl_styles", "style LIKE '$sStyleNo' AND brand_id='$iBrand' AND season_id='$iSeason' AND sub_season_id='$iSubSeason'");
			...
			*/
			...
			$iStylePoints = array( );
			$bEmpty       = false;
			$iIndex       = ((int)getDbValue("MAX(id)", "tbl_style_specs", "style_id='$iStyle' AND point_id='$iPoint' AND version='0'") + 1);
			...
			while (($sRecord = @fgetcsv($hFile, 10000)) !== FALSE)
			{
				...
				$iPoint = getDbValue("id", "tbl_measurement_points", "point_id LIKE '$sPointId' AND `point` LIKE '$sPointName' AND category_id='$iSamplingCategory' AND brand_id IN (436,527,528,529)");
				
				if ($iPoint == 0)
					$iPoint = getDbValue("id", "tbl_measurement_points", "point_id LIKE 'P%' AND `point` LIKE '$sPointName' AND category_id='$iSamplingCategory' AND brand_id IN (436,527,528,529)");
				
				if ($iPoint == 0 && $iStyle >0)
				{
					...
					if ($bFlag == false)
					{
						...
						exit( );
                                ...
                                if ($bFlag == true && $iPoint > 0 && $iStyle >0)//new code for positions setting
                                {
                                    $sSubSql = "";
                                    if(@in_array($sPointId, array("INS1","INSEC")))
                                            $sSubSql = "nature='C', fb_position='2', cr_position='1'";
                                    else if(@in_array($sPointId, array("WAS","WACT","WACB","WAEH","WA1ER","WA1ES","WA2ER","WA2ES","WBW")))
                                            $sSubSql = "nature='C', fb_position='9', cr_position='2'";
                                    else if(@in_array($sPointId, array("HHW1","LHWF1","LHWP1","SEAT1","SEATB1","SEATBP")))
                                            $sSubSql = "nature='C', fb_position='7', cr_position='3'";
                                    else if(@in_array($sPointId, array("LOBT1","LOBT2R","LOBT2S")))
                                            $sSubSql = "nature='FB', fb_position='1'";
                                    else if(@in_array($sPointId, array("THIGHB","MDTH1")))
                                            $sSubSql = "nature='FB', fb_position='3'";
                                    else if(@in_array($sPointId, array("KNFLD1","KNCR1")))
                                            $sSubSql = "nature='FB', fb_position='4'";
                                    else if(@in_array($sPointId, array("FRS1")))
                                            $sSubSql = "nature='FB', fb_position='5'";
                                    else if(@in_array($sPointId, array("ZIPO1","ZIPO2","BFO1")))
                                            $sSubSql = "nature='FB', fb_position='8'";
                                    else {
                                            
                                            $sSubSql = "nature='S', position='$position'";
                                            $position++;
                                    }
                                    ...
                                    {
                                        $myCounter++;
                                        $sSQL  = "UPDATE tbl_style_specs SET $sSubSql where  style_id = '$iStyle' AND point_id = '$iPoint'";
                                        $bFlag = $objDb->execute($sSQL, false);
				...
				foreach ($sSizes as $iSizeIndex => $iSize)
				{
					...
					//$sSQL  = "INSERT INTO tbl_style_specs (id, style_id, point_id, size_id, specs, version, nature) VALUES ('$iIndex', '$iStyle', '$iPoint', '$iSize', '$fSpecsValue', '0', '$sNature')";
					//$bFlag = $objDb->execute($sSQL, false);		
					...
					$iIndex ++;
                                ...
                                /*if(array_sum($CheckDiscardLines) > 0)
                                {
                                    //do nothing
                                }else
                                {
                                    echo "DELETE FROM tbl_style_specs WHERE style_id='$iStyle' AND point_id='$iPoint' AND version='0'"."<br/>";
                                    $sCheckDiscardedPoints[] = $sPointId.":~:".$sPointName;
                                }*/
