<?
	...
	***********************************************************************************************
	...
	$sSQL  = "SELECT * FROM tbl_courier WHERE awb_no LIKE '$AirwayBill' AND id!='$Id'";
	$objDb->query($sSQL);

	if ($objDb->getCount( ) == 0)
	{
		$sSQL = "UPDATE tbl_couriers SET awb_no      = '$AirwayBill',
		                                 `type`      = '$Type',
		                                 user_id     = '$Employee',
		                                 company     = '$Company',
		                                 country_id  = '$Country',
		...
		if ($objDb->execute($sSQL) == true)
			redirect($_SERVER['HTTP_REFERER'], "COURIER_ITEM_UPDATED");
