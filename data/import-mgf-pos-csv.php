	...
	***********************************************************************************************
	...
	else
	{
		...
		while (($sRecord = @fgetcsv($hFile, 10000)) !== FALSE)
		{
			...
			if ($Delete != "Y")
			{
				...
				if ($iStyle == 0 && $iSubBrand > 0)
				{
					...
					$sSQL  = "INSERT INTO tbl_styles (id, category_id, style, style_name, reference, brand_id, sub_brand_id, season_id, sub_season_id, program_id, design_no, design_name, carry_over_id, fabric_width, specs_file, sketch_file, measurement_points, sizes, created, created_by, modified, modified_by)
											  VALUES ('$iStyle', '$iCategory', '$sStyle', '$sStyleDesc', '', '$iMainBrand', '$iSubBrand', '$iMainSeason', '$iSubSeason', '1', '', '', '0', '0', '', '', '', '', NOW( ), '{$_SESSION['UserId']}', NOW( ), '{$_SESSION['UserId']}')";
					$bFlag = $objDb->execute($sSQL, false);
			...
			if ($iFactory == 0 || $iSubBrand == 0 || $iStyle == 0 || $iSize == 0)
				continue;
		...
		if ($bFlag == true && $Delete != "Y")
		{
			...
			while (($sRecord = @fgetcsv($hFile, 10000)) !== FALSE)
			{
				...
				if ($iFactory == 0 || $iStyle == 0 || $iSize == 0 || $iSubBrand == 0)
				{
					if ($iFactory == 0)
						$sOutput = str_replace("[Result]", '<span class="error">Vendor not found</span>', $sOutput);
					
					else if ($iSubBrand == 0)
						$sOutput = str_replace("[Result]", '<span class="error">Brand not found</span>', $sOutput);
				...
				if ($iPoId == 0)
				{
					...
					$sSQL  = "INSERT INTO tbl_po (id, vendor_id, brand_id, order_no, order_status, order_nature, customer_po_no, vpo_no, mgf_vendor, call_no, terms_of_delivery, place_of_departure, way_of_dispatch, terms_of_payment, size_set, lab_dips, photo_sample, pre_prod_sample, note, quantity, styles, sizes, destinations, shipping_dates, status, created, created_by, modified, modified_by)
										  VALUES ('$iPoId', '$iFactory', '$iSubBrand', '$sPerfomaNo', '', 'B', '$sSalesOrderNo', '$sOrderNo', '$sVendor', '', 'N/A', 'N/A', 'N/A', 'N/A', '', '', '', '', '', '$iQuantity', '$iStyle', '$iSize', '$Destination', '$sEtdRequired', 'W', NOW( ), '{$_SESSION['UserId']}', NOW( ), '{$_SESSION['UserId']}')";
					$bFlag = $objDb->execute($sSQL, false);
				...
				{
					$iColorId = (int)getDbValue("id", "tbl_po_colors", "po_id='$iPoId' AND style_id='$iStyle' AND color LIKE '$sColor' AND line LIKE '$sLine' AND destination_id='$Destination' AND etd_required='$sEtdRequired'");
...
*/
		...
		if ($bFlag == true)
		{
			...
			$sSQL  = "UPDATE tbl_po SET quantity       = (SELECT SUM(quantity) FROM tbl_po_quantities WHERE po_id=tbl_po.id),
					 ...
					 WHERE FIND_IN_SET(brand_id, '$sBrands') AND DATE(modified)=CURDATE( ) AND modified_by='{$_SESSION['UserId']}'";
			$bFlag = $objDb->execute($sSQL, false);
		}		
