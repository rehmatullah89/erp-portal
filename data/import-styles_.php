...
//                                            if (@in_array($sPointId, array("INS1","INSEC")))
			...
			if ($bFlag == true)
			{
				//$sSQL  = ("UPDATE tbl_styles SET measurement_points='".@implode(",", $iStylePoints)."', sizes='".@implode(",", $iStyleSizes)."' WHERE id='$iStyle'");
                                ...
                                $sSQL  = ("UPDATE tbl_styles SET measurement_points=(SELECT GROUP_CONCAT(DISTINCT(point_id) SEPARATOR ',') FROM tbl_style_specs WHERE style_id=tbl_styles.id AND version='0'), sizes='".@implode(",", $iStyleSizes)."' WHERE id='$iStyle'");
				$bFlag = $objDb->execute($sSQL, false);					
