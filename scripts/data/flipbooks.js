...
document.observe('dom:loaded', function( )
{
	...
	for (var i = 0; i < objProducts.options.length; i ++)
		sProducts[i] = new Array(objProducts.options[i].text, objProducts.options[i].value, objProducts.options[i].getAttribute("rel"));
...
function showSelectedImage( )
{
	...
	if (sValue != "")
	{
		...
		{
			if (sProducts[i][1] == sValue)
			{
				$("Image").src = sProducts[i][2];
...
function validateForm( )
{
	...
	if (!objFV.validate("FrontPicture", "B", "Please enter the Front Picture."))
		return false;
	...
	if (!objFV.validate("BackPicture", "B", "Please enter the Back Picture."))
		return false;
