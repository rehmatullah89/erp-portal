...
function validateForm( )
{
	...
	if (!objFV.validate("LoomType", "B", "Please enter the Loom Type Title."))
		return false;

	if (!objFV.validate("Capacity", "B,F", "Please enter the Loom Capacity (per hour)."))
		return false;
...
function validateEditForm(iId)
{
	...
	if (!objFV.validate("Capacity", "B,F", "Please enter the Loom Capacity."))
		return false;


	$('Processing').show( );

	var sUrl    = "ajax/yarn/update-loom-type.php";
		...
		if (sParams[0] == "OK")
		{
			...
			setTimeout(
				    function( )
				    {
						...
						$('LoomType' + iId).innerHTML = sParams[3];
						$('Capacity' + iId).innerHTML = sParams[4];
						},
