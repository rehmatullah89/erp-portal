...
function validateForm( )
{
	...
	if (!objFV.validate("DefectCode", "B", "Please enter Defect Code."))
		return false;
            
        if (!objFV.validate("Defect", "B", "Please enter Defect."))
		return false;
...
function validateEditForm(iId)
{
	...
	var sUrl    = "ajax/quonda/update-packaging-defect.php";
		...
		if (sParams[0] == "OK")
		{
			...
			setTimeout(
					...
					$('Code' + iId).innerHTML   = sParams[3];
                                        $('Defect' + iId).innerHTML = sParams[4];
