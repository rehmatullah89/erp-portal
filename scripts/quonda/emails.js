...
function validateForm( )
{
	...
	if (!objFV.validate("Email", "B,E", "Please enter the Email."))
		return false;

	if (objFV.selectedIndex("Vendors") == -1)
	{
...
function validateEditForm(iId)
{
	...
	if (objFV.selectedIndex("Vendors" + iId) == -1)
	{
		alert("Please select at-least one Vendor");
	...
	var sUrl    = "ajax/quonda/update-email.php";
	...
	if (sResponse.status == 200 && sResponse.statusText == "OK")
	{
		...
		if (sParams[0] == "OK")
		{
			...
			setTimeout(
					...
					$('AuditStages_' + iId).innerHTML = sParams[4];
					$('Name' + iId).innerHTML         = sParams[5];
					$('Email' + iId).innerHTML        = sParams[6];
				    },
