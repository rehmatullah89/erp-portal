...
function validateForm( )
{
	...
	if (!objFV.validate("ParentStage", "B", "Please Select a Parent Stage."))
		return false;
...
function validateEditForm(iId)
{
	...
	if (!objFV.validate("ParenStage", "B", "Please Select a Parent Stage."))
		return false;


	$('Processing').show( );

	var sUrl    = "ajax/quonda/update-audit-sub-stage.php";
	...
	if (sResponse.status == 200 && sResponse.statusText == "OK")
	{
		...
		if (sParams[0] == "OK")
		{
			...
			setTimeout(
					...
					$('Parent' + iId).innerHTML       = sParams[4];
				    },
