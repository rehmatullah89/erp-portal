...
function validateForm( )
{
	...
	if (!objFV.validate("Item", "B", "Please enter the Checklist Item Name."))
		return false;
            
       	if (!objFV.validate("FieldType", "B", "Please Select the field type."))
		return false;   
            
        if (!objFV.validate("Mandatory", "B", "Please select the Field if mandatory."))
		return false;               
...
function validateEditForm(iId)
{
	...
	var sUrl    = "ajax/quonda/update-qa-checklist.php";
	...
	if (sResponse.status == 200 && sResponse.statusText == "OK")
	{
		...
		if (sParams[0] == "OK")
		{
			...
			setTimeout(
					...
					$('Item' + iId).innerHTML   = sParams[3];
                                        $('FieldType' + iId).innerHTML   = sParams[4];
                                        $('Mandatory' + iId).innerHTML   = sParams[5];
				    },
