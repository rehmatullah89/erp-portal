...
function validateForm( )
{
	...
	if (!objFV.validate("Area", "B", "Please select the Area."))
		return false;

	if (!objFV.validate("Section", "B", "Please enter the Section."))
		return false;

	if (!objFV.validate("Point", "B", "Please enter the Quality Point."))
		return false;
...
function validateEditForm(iId)
{
	...
	var sUrl    = "ajax/crc/update-quality-point.php";
	...
	if (sResponse.status == 200 && sResponse.statusText == "OK")
	{
		...
		if (sParams[0] == "OK")
		{
			...
			setTimeout(
					...
					$('Area_' + iId).innerHTML   = sParams[3];
					$('Section' + iId).innerHTML = sParams[4];
					$('Point_' + iId).innerHTML  = sParams[5];
				    },
