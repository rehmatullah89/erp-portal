...
?>
				  <h2><?= $sVendorsList[$Vendor] ?> Audits (<?= formatDate($FromDate) ?> to <?= formatDate($ToDate) ?>)</h2>
...
<?
	...
	$sSQL = "SELECT DISTINCT(audit_stage) FROM tbl_qa_reports WHERE (audit_date BETWEEN '$FromDate' AND '$ToDate') $sConditions";
	$objDb->query($sSQL);
...
<?
	$sSQL = "SELECT COUNT(1) FROM tbl_qa_reports WHERE (audit_date BETWEEN '$FromDate' AND '$ToDate') $sConditions GROUP BY audit_date, user_id";
	$objDb->query($sSQL);

	$iRows = $objDb->getCount( );


	$sSQL = "SELECT COUNT(1) FROM tbl_qa_reports WHERE (audit_date BETWEEN '$FromDate' AND '$ToDate') $sConditions GROUP BY audit_date";
	$objDb->query($sSQL);
...
?>
						  <tr>
							<td align="right" bgcolor="#e6e6e6"><b><?= formatDate($sDate) ?></b></td>
