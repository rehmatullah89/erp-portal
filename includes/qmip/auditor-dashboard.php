...
<?
	...
	$sSQL = "SELECT DISTINCT(audit_stage) FROM tbl_qa_reports WHERE (audit_date BETWEEN '$FromDate' AND '$ToDate') AND (user_id='$Auditor' OR (group_id>'0' AND group_id IN (SELECT id FROM tbl_auditor_groups WHERE FIND_IN_SET('$Auditor', users)))) $sConditions";
	$objDb->query($sSQL);
...
<?
		...
		$sSQL = "SELECT id, TIME_TO_SEC(start_time) AS _StartTime, TIME_TO_SEC(end_time) AS _EndTime, audit_date, audit_code, vendor_id, brand_id, style_id, po_id,
