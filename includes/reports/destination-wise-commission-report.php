<?
	...
	***********************************************************************************************
	...
	if (count($Brand) > 0)
		$sSQL = ("SELECT id, destination FROM tbl_destinations WHERE brand_id=(SELECT parent_id FROM tbl_brands WHERE id IN (".@implode(",", $Brand).")) ORDER BY destination");

	else
		$sSQL = "SELECT id, destination FROM tbl_destinations ORDER BY destination";
	...
	for ($i = 0; $i < $iCount; $i ++)
	{
		$iDestinationId = $objDb->getField($i, 0);
		$sDestination   = $objDb->getField($i, 1);
			...
			for ($k = 0; $k < $iCount3; $k ++)
			{
				...
				for ($l = 0; $l < $iCount4; $l ++)
				{
					...
					if ($iQuantity > 0)
					{
						...
						$objPHPExcel->getActiveSheet()->setCellValue('B'.$iIndex, $sDestination);
		...
		if ($iDestinationPcs > 0)
		{
			$objPHPExcel->getActiveSheet()->setCellValue('N'.$iIndex, "Destination Total ({$sDestination}) :");
			$objPHPExcel->getActiveSheet()->mergeCells('N'.$iIndex.':P'.$iIndex);

			$objPHPExcel->getActiveSheet()->setCellValue('Q'.$iIndex, formatNumber($iDestinationPcs, false));
			$objPHPExcel->getActiveSheet()->setCellValue('R'.$iIndex, ("{$sLeftCurrency}".formatNumber($fDestinationPrice, true, $iDecimals)."{$sRightCurrency}"));

			if ($sUserRights['Add'] == "Y" && $sUserRights['Edit'] == "Y" && $sUserRights['Delete'] == "Y")
				$objPHPExcel->getActiveSheet()->setCellValue('T'.$iIndex, ("{$sLeftCurrency}".formatNumber($fDestinationCommission, true, $iDecimals)."{$sRightCurrency}"));
		...
		$iTotalPcs        += $iDestinationPcs;
		$fTotalPrice      += $fDestinationPrice;
		$fTotalCommission += $fDestinationCommission;
	}
