	...
	***********************************************************************************************
	...
	for ($i = 0; $i < $iCount; $i ++)
	{
		...
		for ($j = 0; $j < $iCount2; $j ++)
		{
			...
			$sSQL = "SELECT id, color, line, price, style_id, etd_required FROM tbl_po_colors WHERE po_id='$iPoId' ORDER BY color";
			$objDb3->query($sSQL);
		...
		if ($iInvoicePcs > 0)
		{
			...
			$objPHPExcel->getActiveSheet()->mergeCells('N'.$iIndex.':O'.$iIndex);
