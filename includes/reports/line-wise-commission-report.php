<?
	...
	***********************************************************************************************
	...
	$objPHPExcel->getActiveSheet()->setCellValue('H12', 'CR Date');
	$objPHPExcel->getActiveSheet()->setCellValue('I12', 'FCR Date');
	$objPHPExcel->getActiveSheet()->setCellValue('J12', 'Shipping Date');
	$objPHPExcel->getActiveSheet()->setCellValue('K12', 'Arrival Date');
	$objPHPExcel->getActiveSheet()->setCellValue('L12', 'Terms of Payment');
	$objPHPExcel->getActiveSheet()->setCellValue('M12', 'Airway/Ladding Bill');
	$objPHPExcel->getActiveSheet()->setCellValue('N12', 'Invoice #');
	$objPHPExcel->getActiveSheet()->setCellValue('O12', 'Price');
	$objPHPExcel->getActiveSheet()->setCellValue('P12', 'Ship Qty');
	$objPHPExcel->getActiveSheet()->setCellValue('Q12', 'Amount');

	if ($sUserRights['Add'] == "Y" && $sUserRights['Edit'] == "Y" && $sUserRights['Delete'] == "Y")
	{
		$objPHPExcel->getActiveSheet()->setCellValue('R12', 'Rate');
		$objPHPExcel->getActiveSheet()->setCellValue('S12', 'Commission');
	...
	$sSQL = "SELECT DISTINCT(line) FROM tbl_po_colors WHERE line!='' AND NOT ISNULL(line) AND FIND_IN_SET(po_id, '$sPos') ORDER BY line";
	$objDb->query($sSQL);
	...
	for ($i = 0; $i < $iCount; $i ++)
	{
		...
		$sSQL =" SELECT DISTINCT(po_id) FROM tbl_po_colors WHERE line='$sLine'";
		$objDb2->query($sSQL);
		...
		for ($j = 0; $j < $iCount2; $j ++)
		{
			...
			for ($k = 0; $k < $iCount3; $k ++)
			{
				...
				$sSQL = "SELECT id, color, price, style_id, etd_required FROM tbl_po_colors WHERE po_id='$iPoId' AND line='$sLine' ORDER BY color";
				$objDb4->query($sSQL);
				...
				for ($l = 0; $l < $iCount4; $l ++)
				{
					...
					$sSQL = "SELECT style, season_id AS _Season FROM tbl_styles WHERE id='$iStyleId'";
					$objDb5->query($sSQL);
					...
					if ($iQuantity > 0)
					{
						...
						$objPHPExcel->getActiveSheet()->setCellValue('L'.$iIndex, $sTermsOfPayment);
						$objPHPExcel->getActiveSheet()->setCellValue('M'.$iIndex, $sLadingAirwayBill);
						$objPHPExcel->getActiveSheet()->setCellValue('N'.$iIndex, " $sInvoiceNo");
						$objPHPExcel->getActiveSheet()->setCellValue('O'.$iIndex, ("{$sLeftCurrency}".formatNumber($fPrice, true, $iDecimals)."{$sRightCurrency}"));
						$objPHPExcel->getActiveSheet()->setCellValue('P'.$iIndex, formatNumber($iQuantity, false));
						$objPHPExcel->getActiveSheet()->setCellValue('Q'.$iIndex, ("{$sLeftCurrency}".formatNumber($fAmount, true, $iDecimals)."{$sRightCurrency}"));

						if ($sUserRights['Add'] == "Y" && $sUserRights['Edit'] == "Y" && $sUserRights['Delete'] == "Y")
						{
							$objPHPExcel->getActiveSheet()->setCellValue('R'.$iIndex, formatNumber($fCommission, (($sCommissionType == "P") ? true : false)).(($sCommissionType == "P") ? "%" : @utf8_encode("¢")));
							$objPHPExcel->getActiveSheet()->setCellValue('S'.$iIndex, ("{$sLeftCurrency}".formatNumber($fMatrixCommission)."{$sRightCurrency}"));
						}
			...
			if ($iInvoicePcs > 0)
			{
				$objPHPExcel->getActiveSheet()->setCellValue('M'.$iIndex, "Invoice Total :");
				$objPHPExcel->getActiveSheet()->mergeCells('M'.$iIndex.':O'.$iIndex);

				$objPHPExcel->getActiveSheet()->setCellValue('P'.$iIndex, formatNumber($iInvoicePcs, false));
				$objPHPExcel->getActiveSheet()->setCellValue('Q'.$iIndex, ("{$sLeftCurrency}".formatNumber($fInvoicePrice, true, $iDecimals)."{$sRightCurrency}"));

				if ($sUserRights['Add'] == "Y" && $sUserRights['Edit'] == "Y" && $sUserRights['Delete'] == "Y")
					$objPHPExcel->getActiveSheet()->setCellValue('S'.$iIndex, ("{$sLeftCurrency}".formatNumber($fInvoiceCommission, true, $iDecimals)."{$sRightCurrency}"));
			...
			$iLinePcs        += $iInvoicePcs;
			$fLinePrice      += $fInvoicePrice;
			$fLineCommission += $fInvoiceCommission;
		}


		if ($iLinePcs > 0)
		{
			$objPHPExcel->getActiveSheet()->setCellValue('M'.$iIndex, "Line Total ({$sLine}) :");
			$objPHPExcel->getActiveSheet()->mergeCells('M'.$iIndex.':O'.$iIndex);

			$objPHPExcel->getActiveSheet()->setCellValue('P'.$iIndex, formatNumber($iLinePcs, false));
			$objPHPExcel->getActiveSheet()->setCellValue('Q'.$iIndex, ("{$sLeftCurrency}".formatNumber($fLinePrice, true, $iDecimals)."{$sRightCurrency}"));

			if ($sUserRights['Add'] == "Y" && $sUserRights['Edit'] == "Y" && $sUserRights['Delete'] == "Y")
				$objPHPExcel->getActiveSheet()->setCellValue('S'.$iIndex, ("{$sLeftCurrency}".formatNumber($fLineCommission, true, $iDecimals)."{$sRightCurrency}"));
		...
		$iTotalPcs        += $iLinePcs;
		$fTotalPrice      += $fLinePrice;
		$fTotalCommission += $fLineCommission;
	}


	if ($iTotalPcs > 0)
	{
		$objPHPExcel->getActiveSheet()->setCellValue('M'.$iIndex, "Grand Shipment Total :");
		$objPHPExcel->getActiveSheet()->mergeCells('M'.$iIndex.':O'.$iIndex);

		$objPHPExcel->getActiveSheet()->setCellValue('P'.$iIndex, formatNumber($iTotalPcs, false));
		$objPHPExcel->getActiveSheet()->setCellValue('Q'.$iIndex, ("{$sLeftCurrency}".formatNumber($fTotalPrice, true, $iDecimals)."{$sRightCurrency}"));

		if ($sUserRights['Add'] == "Y" && $sUserRights['Edit'] == "Y" && $sUserRights['Delete'] == "Y")
			$objPHPExcel->getActiveSheet()->setCellValue('S'.$iIndex, ("{$sLeftCurrency}".formatNumber($fTotalCommission, true, $iDecimals)."{$sRightCurrency}"));
