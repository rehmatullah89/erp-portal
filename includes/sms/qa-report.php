<?
	...
	***********************************************************************************************
	...
	$sAuditCode    = strtoupper($sFields[0]);
	$sPO           = strtoupper($sFields[1]);
	$sAuditStage   = strtoupper($sFields[2]);
	$sAuditStatus  = strtolower($sFields[3]);
	$iSampleSize   = intval($sFields[4]);
	$sColors       = $sFields[5];
	$sAuditResult  = strtoupper($sFields[6]);
	...
	if ($bFlag == true)
	{
		...
		$sSQL  = "UPDATE tbl_qa_reports SET po_id='$iPoId', style_id='$iStyleId', audit_stage='$sAuditStage', audit_status='$sAuditStatus', total_gmts='$iSampleSize', colors='$sColors', audit_result='$sAuditResult', ship_qty='$iShipQty', re_screen_qty='$iReScreenQty', qa_comments='$sComments', date_time=NOW( ), audit_mode='3' WHERE id='$iAuditId'";
