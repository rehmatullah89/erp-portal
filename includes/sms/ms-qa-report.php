<?
	...
	***********************************************************************************************
	...
	if ($sAuditCode == "" || $sPO == "" || !@in_array($sAuditStage, $sAuditStages) ||
	    $iSampleSize == 0 || $sColors == "" || !@in_array($sAuditResult, array("P", "F", "H"))  || $sComments == "") // || $iBatchSize == 0
	{
...
//			$sSubject = "--- Invalid Batch Size ---";
	...
	if ($bFlag == true)
	{
		...
		$sSQL  = "UPDATE tbl_qa_reports SET po_id='$iPoId', style_id='$iStyleId', audit_stage='$sAuditStage', total_gmts='$iSampleSize', colors='$sColors', batch_size='$iBatchSize', audit_result='$sAuditResult', qa_comments='$sComments', date_time=NOW( ), audit_mode='3' WHERE id='$iAuditId'";
