<?
	...
	***********************************************************************************************
	...
	$sSQL = "SELECT *,
	         ...
	         FROM tbl_inline_audits WHERE id='$Id'";
	$objDb->query($sSQL);
...
*/
	...
	$objPdf->Text(135, 26.3, "");
	$objPdf->Text(135, 28.8, $fPackedPercent);
	$objPdf->Text(135, 31.4, $iTotalGmts);
	...
	for ($i = 0; $i < count($sCodes); $i ++)
	{
		...
		$iCriticalDefects = getDbValue("SUM(defects)", "tbl_inline_audit_defects", "nature='2' AND audit_id='$Id' AND code_id='$iCode'");
		$iMajorDefects    = getDbValue("SUM(defects)", "tbl_inline_audit_defects", "nature='1' AND audit_id='$Id' AND code_id='$iCode'");
		$iMinorDefects    = getDbValue("SUM(defects)", "tbl_inline_audit_defects", "nature='0' AND audit_id='$Id' AND code_id='$iCode'");


		$sPosition = "";

		$sSQL = "SELECT area FROM tbl_sampling_defect_areas WHERE id IN (SELECT area_id FROM tbl_inline_audit_defects WHERE audit_id='$Id' AND code_id='$iCode')";
		$objDb->query($sSQL);
	...
	$objPdf->Text(126, 224, ($iTotalGmts - $iMajor - $iMinor));
	$objPdf->Text(126, 226.3, ($iMajor + $iMinor));
	$objPdf->Text(126, 228.6, $iTotalGmts);

	if ($sAuditResult == "P")
		$objPdf->Text(126, 233.5, 'Pass');

	else
		$objPdf->Text(126, 233.5, 'Fail');
	...
	$sPdfFile = ($sBaseDir.TEMP_DIR."M{$Id}-M&S-QA-Report.pdf");
