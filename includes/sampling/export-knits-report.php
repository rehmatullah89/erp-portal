<?
	...
	***********************************************************************************************
	...
	$iPageCount  = $objPdf->setSourceFile($sBaseDir."templates/knits-old.pdf");
	...
	$objPdf->Text(24, 52.3, $sBrand);
	$objPdf->Text(80, 52.5, $sVendor);
	$objPdf->Text(118, 52.5, formatDate($sAuditDate));
	$objPdf->Text(185.5, 52.5, $sAuditor);

	$objPdf->Text(15, 59.5, "");
	$objPdf->Text(84.5, 59.5, $sStyle);
	$objPdf->Text(125, 59.5, "");
	$objPdf->Text(185.5, 59.5, formatNumber($iShipQty, false));

	$objPdf->Text(27, 66.5, "");
	$objPdf->Text(82, 66.5, $sSizeTitles);
	$objPdf->Text(122, 66.5, $sColors);
	$objPdf->Text(185.5, 66.5, formatNumber($fCartonsShipped, false));

	$objPdf->Text(28, 74.5, "");


  	if ($sAuditStatus == "1st")
		$objPdf->Image(($sBaseDir.'images/icons/tick.gif'), 140, 73.0, 4);

  	else if ($sAuditStatus == "2nd")
		$objPdf->Image(($sBaseDir.'images/icons/tick.gif'), 150, 73.0, 4);

  	else if ($sAuditStatus == "3rd")
		$objPdf->Image(($sBaseDir.'images/icons/tick.gif'), 160, 73.0, 4);

  	else
		$objPdf->Text(167, 75, $sAuditStatus);


	$objPdf->Text(190, 75, strtoupper($sAuditStage));
	...
	$objPdf->Text(33, 93, $sStartTime);
	$objPdf->Text(74, 93, $sEndTime);
	$objPdf->Text(110, 93, (($iSubBrand == 132) ? "4.0" : "2.5"));


	if ($sAuditResult == "P")
		$objPdf->Text(202, 93, 'PASS');

	else if ($sAuditResult == "F")
		$objPdf->Text(202, 93, 'FAIL');

	else
		$objPdf->Text(202, 93, 'HOLD');
	...
	for ($i = 0; $i < count($sCodes); $i ++)
	{
		...
		$objPdf->Text( (($i <= 33) ? 77 : 190), (($i <= 33) ? (103.2 + (3.64 * $i)) : (99.6 + (3.64 * ($i - 33)))), $iDefects);
	}
	...
	$objPdf->Text(48, 231.5, $iTotalGmts);
	$objPdf->Text(88, 231.5, (($iGmtsDefective > 0) ? $iGmtsDefective : $iDefects));
	$objPdf->Text(128, 231.5, $iMaxDefects);
	$objPdf->Text(168, 231.5, $iDefects);
	$objPdf->Text(195, 231.5, @round((($iDefects / $iTotalGmts) * 100), 2));

	$objPdf->Text(48, 240, $iTotalCartons);
	$objPdf->Text(88, 240, $iCartonsRejected);
	$objPdf->Text(128, 240, $fStandard);
	$objPdf->Text(168, 240, $fPercentDecfective);
	$objPdf->Text(195, 240, @round((($fCartonsShipped / $fCartonsRequired) * 100), 2));


  	if ($sApprovedSample == "Yes")
		$objPdf->Image(($sBaseDir.'images/icons/tick.gif'), 80, 242, 4);

  	else if ($sApprovedSample == "No")
		$objPdf->Image(($sBaseDir.'images/icons/tick.gif'), 93, 242, 4);

	$objPdf->Text(136, 245.7, (($sShippingMark == "Y") ? "Yes" : "No"));
	$objPdf->Text(188, 245.8, (($sPackingCheck == "Y") ? "Yes" : "No"));

	$objPdf->Text(181, 250, $iLength);
	$objPdf->Text(193, 250, $iWidth);
	$objPdf->Text(203, 250, $iHeight);


	$objPdf->SetFont('Arial', '', 5.5);

	$objPdf->SetXY(8.5, 258);
