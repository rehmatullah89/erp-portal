...
*/
	...
	{
		$sSQL = "SELECT defects,
		                (SELECT buyer_code FROM tbl_sampling_defect_codes WHERE id=tbl_inline_audit_defects.code_id) AS _Code
		         FROM tbl_inline_audit_defects WHERE audit_id='$Id' AND code_id IN (SELECT id FROM tbl_sampling_defect_codes WHERE type_id='{$iTypes[$i]}') ORDER BY id";
		$objDb->query($sSQL);
	...
	$objPdf->Text(152, 196, $sProductionStatus);
	...
	$sPdfFile = ($sBaseDir.TEMP_DIR."M{$Id}-Adidas-QA-Report.pdf");
