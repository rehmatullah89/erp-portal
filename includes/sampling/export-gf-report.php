<?
	...
	***********************************************************************************************
	...
	$sSQL = "SELECT * FROM tbl_inline_audits WHERE id='$Id'";
	$objDb->query($sSQL);
...
/*
	...
	for ($i = 0; $i < $iCount; $i ++)
	{
		...
		{
			$sSQL = "SELECT SUM((defects * grade)), SUM(defects) FROM tbl_gf_report_defects WHERE audit_id='$Id' AND roll='$iRoll' AND code_id IN (SELECT id FROM tbl_sampling_defect_codes WHERE report_id='$iReportId' AND type_id='{$iTypes[$j]}')";
			$objDb3->query($sSQL);
...
*/
	...
	$objPdf->Text(165, 59.8, formatNumber($fDhu, 2));
...
/*
	...
	for ($i = 0; $i < count($sCodes); $i ++)
	{
		...
		$iCode = (int)getDbValue("id", "tbl_sampling_defect_codes", "report_id='$iReportId' AND code='{$sCodes[$i]}'");
	...
	if ($iPoints2 > 0)
		$objPdf->Text(109.5, (42.7 + (3.44 * $i)), formatNumber((($iPoints1*3600)/$fActual2)/$iFabricWidth) );

	if ($iPoints3 > 0)
		$objPdf->Text(151, (42.7 + (3.44 * $i)), formatNumber((($iPoints1*3600)/$fActual3)/$iFabricWidth) );
...
*/
	...
	$sPdfFile = ($sBaseDir.TEMP_DIR."M{$Id}-Greige-Fabric-QA-Report.pdf");
