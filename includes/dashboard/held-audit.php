<?
	...
	***********************************************************************************************
	\*********************************************************************************************/
?>
          <h2 style="background:#ff8400; font-size:21px; font-weight:normal; text-align:center; color:#ffffff; padding:8px; margin:15px 0px 2px 0px;">HELD AUDITS</h2>
<?
	$sConditions = " AND (qa.audit_date BETWEEN '$sFromDate' AND '$sToDate') AND qa.approved='Y' AND NOT FIND_IN_SET(qa.report_id, '$sQmipReports') AND qa.audit_result='H' AND NOT FIND_IN_SET(qa.report_id,'4,6,12') ";
	...
	$sSQL = "SELECT qa.id, qa.audit_code, qa.audit_date, qa.audit_stage, s.style, s.sketch_file
			 ...
			 WHERE qa.style_id=s.id $sConditions
			 ORDER BY qa.id DESC
			 LIMIT 12";
	$objDb->query($sSQL);
