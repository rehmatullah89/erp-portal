...
?>
                        <div class="img-container">
                            <a href="<?= ($sSpecsDir.$SpecsSheet) ?>" class="lightview"> <img src="<?= ($sSpecsDir.$SpecsSheet) ?>" width="100" height="100"/></a><br/><?=$i+1?> &nbsp;-&nbsp;<a href="./" class="deleteSpecs" rel="<?= $Id ?>" index="<?= $i+1 ?>"><b>x</b></a> 
                        </div>
<?
			}
                    }
...
?>
                        <div class="img-container">   
                            <a href="<?= ($SectionId == 'L')?$sSpecsDir:$sPicsDir ?><?= $sPicture ?>" class="lightview"><img src="<?= ($SectionId == 'L')?$sSpecsDir:$sPicsDir ?><?= $sPicture ?>" width="100" height="100"/></a> <br/> <a href="./" file="<?= $sPicture ?>" date="<?= $AuditDate ?>" audit_id="<?=$Id?>" image_id="<?=$iPicture?>" class="deletePic"><b>x</b></a>
                        </div>
<?
			}
		}
...
<script type="text/javascript">
  <!--
    ...
    function validateForm( )
    {
            ...
            if (!objFV.validate("filePictures[]", "B", "Please select image(s) first."))
                    return false;

            var selection = document.getElementById('filePictures');
            for (var i=0; i<selection.files.length; i++) {
                ...
                if(ext!== "jpg" && ext!== "jpeg")  
                {
                    alert('Please select only JPG file types.');
                    return false;
                }
            }         
