<?
	...
	***********************************************************************************************
	...
	$iPageCount  = $objPdf->setSourceFile($sBaseDir."templates/apparel.pdf");
	...
	$objPdf->Text(4, 30, "Page 1 of {$iTotalPages}");
	...
	$objPdf->Text(116, 44.0, formatDate($sAuditDate));
	$objPdf->Text(183.5, 44.0, $sAuditor);

	$objPdf->Text(12, 50.8, ($sPo.$sAdditionalPos));
	$objPdf->Text(85.5, 50.8, $sStyle);
	$objPdf->Text(124, 50.8, formatNumber($iQuantity, false));
	$objPdf->Text(183.5, 50.8, formatNumber($iShipQty, false));

	$objPdf->Text(23, 57.5, $sDestination);
	$objPdf->Text(84.5, 57.5, $sSizeTitles);

	$objPdf->SetFont('Arial', '', 6);
	$objPdf->SetXY(119, 55.5);
	$objPdf->MultiCell(90, 3.0, $sColors, 0);


	$objPdf->SetFont('Arial', '', 7);
	$objPdf->Text(22, 64, $sCustomer);
	$objPdf->Text(86, 64, $sSeason);
	$objPdf->Text(121.5, 64, $sProgram);
	$objPdf->Text(183.5, 64, formatNumber($fCartonsShipped, false));

	$objPdf->Text(22, 70.5, $sDesignNo);
	$objPdf->Text(94, 70.5, $sDesignName);


	$objPdf->Text(25, 79.0, $sDescription);
	$objPdf->Text(140, 79.5, $sAuditStatus);
	$objPdf->Text(176, 79.5, strtoupper($sAuditStage));
	...
	$objPdf->Text(193, 228.5, @round((($iTotalMajors / $iTotalGmts) * 100), 2));
