...
?>
				  <h2>
				    <span style="float:right; padding-right:5px;">[ <a href="quonda/dashboard.php?Region=<?= $Region ?>&FromDate=<?= date("Y-m-d") ?>" style="color:#ffff00;">Today's Audits</a> ]</span>
				    Recent Audits
				  </h2>
...
<?
	...
	$sSQL = "SELECT DISTINCT(audit_date) AS _Date, GROUP_CONCAT(user_id SEPARATOR ',') AS _Auditors
	         FROM tbl_qa_reports
	         $sConditions AND audit_date<=CURDATE( )
	         GROUP BY audit_date
	...
	$sAuditDates  = @implode(",", $sAuditDates);
	$sConditions .= " AND FIND_IN_SET(audit_date, '$sAuditDates') ";
...
?>
						  <tr>
							<td align="right"><span style="display:block; height:35px; overflow:hidden;"><?= getDbValue("name", "tbl_users", "id='$iAuditor'") ?></span></td>
...
//						$sEndTime    = date("H:i:s", (strtotime($sEndTime) + 10800));
					...
					if ($iReportId == 10)
					{
						$iDefects     = $objDb->getField($j, '_JkDefects');
						$iDefectPics  = $objDb->getField($j, '_JkDefectPics');
					}
