<?
	...
	***********************************************************************************************
	...
	if (strlen($sSizeTitle) > 75)
		$sSizeTitle = (substr($sSizeTitle, 0, 72)."...");
	...
	$iPageCount  = $objPdf->setSourceFile($sBaseDir."templates/woven-p1.pdf");
	...
	$objPdf->Text(13, 45.5, ($sPo.$sAdditionalPos));
	$objPdf->Text(83, 45.5, $sStyle);
	$objPdf->Text(140, 45.5, formatNumber($iQuantity, false));
	$objPdf->Text(185, 45.5, formatNumber($iShipQty, false));

	$objPdf->Text(24, 52.5, $sDestination);
	$objPdf->Text(195, 52.5, formatNumber($fCartonsShipped, false));

	$objPdf->SetXY(78, 50);
	$objPdf->MultiCell(24, 3.2, $sSizeTitles, 0);

	$objPdf->SetXY(116, 50);
	$objPdf->MultiCell(57, 3.2, $sColors, 0);


	$objPdf->Text(26, 60.3, $sDescription);
	$objPdf->Text(140, 61.5, $sAuditStatus);
	$objPdf->Text(175, 61.5, strtoupper($sAuditStage));


	$objPdf->Text(33, 74, (($fKnitted > 0) ? $fKnitted : "N/A"));
	$objPdf->Text(54, 74, (($fDyed > 0) ? $fDyed : "N/A"));
	$objPdf->Text(77, 74, (($iCutting > 0) ? formatNumber($iCutting, false) : "N/A"));
	$objPdf->Text(99, 74, (($iSewing > 0) ? formatNumber($iSewing, false) : "N/A"));
	$objPdf->Text(137, 74, ((($iFinishing > 0) ? formatNumber($iFinishing, false) : "N/A")." / ".(($iPacking > 0) ? formatNumber($iPacking, false) : "N/A")));
	$objPdf->Text(177, 74, (($sFinalAuditDate != "0000-00-00") ? formatDate($sFinalAuditDate) : "N/A"));
	...
	$objPdf->Text(33, 79.2, $sStartTime);
	$objPdf->Text(74, 79.2, $sEndTime);
	$objPdf->Text(112, 79.2, formatNumber($fAql, true, (($fAql < 1) ? 2 : 1)));

	$objPdf->SetFont('Arial', 'B', 7);
	$objPdf->Text(177, 79.2, (($sAuditResult == "P") ? "PASS" : (($sAuditResult == "F") ? "FAIL" : "HOLD")));
	...
	for ($i = 0; $i < count($sCodes); $i ++)
	{
		...
		if ($iMajorDefects > 0)
			$objPdf->Text( (($i <= 38) ? 75 : 188), $fTop, $iMajorDefects);

		if ($iMinorDefects > 0)
			$objPdf->Text( (($i <= 38) ? 87 : 199), $fTop, $iMinorDefects);
	...
	$objPdf->Text(188, 227.8, $iTotalMajors);
	$objPdf->Text(199, 227.8, $iTotalMinors);


	$objPdf->Text(40, 235.4, (($sCustomSample == "Y") ? "CUSTOM ({$iTotalGmts})" : $iTotalGmts));
	$objPdf->Text(88, 235.4, (($iGmtsDefective > 0) ? $iGmtsDefective : $iTotalMajors));
	$objPdf->Text(128, 235.4, $iMaxDefects);
	$objPdf->Text(168, 235.4, ($iTotalMajors + $iTotalMinors));
	$objPdf->Text(195, 235.4, @round(((($iTotalMajors + $iTotalMinors) / $iTotalGmts) * 100), 2));

	$objPdf->Text(48, 242.4, $iTotalCartons);
	$objPdf->Text(88, 242.4, $iCartonsRejected);
	$objPdf->Text(128, 242.4, $fStandard);
	$objPdf->Text(168, 242.4, $fPercentDecfective);
	$objPdf->Text(195, 242.4, @round((($fCartonsShipped / $fCartonsRequired) * 100), 2));
	...
	$objPdf->Text(133, 248.3, (($sShippingMark == "Y") ? "Yes" : "No"));
	$objPdf->Text(188, 248.3, (($sPackingCheck == "Y") ? "Yes" : "No"));


	$objPdf->Text(177, 252.5, $iLength);
	$objPdf->Text(189, 252.5, $iWidth);
	$objPdf->Text(199, 252.5, $iHeight);
