...
<?
			$sSQL = "SELECT qrs.sample_no, qrss.point_id, qrss.findings, qrss.specs
					 ...
					 WHERE qrs.id=qrss.sample_id AND qrs.audit_id='$AuditId' AND qrs.sample_no= '$SampleNo' AND qrs.size_id='$iSize' AND qrs.color='$sColor'
...
<?
			$sSQL = "SELECT point_id, specs, nature,
					 ...
					 WHERE style_id='$Style' AND size_id='$iSize' AND version='0'
...
?>
				  	...
				  	<td align="center"><input type="text" name="Specs<?= $iSize ?>_<?= $iColor ?>_<?= $iPoint ?>" value="<?= $sSizeFindings["{$iPoint}"] ?>" size="4" maxlength="10" class="textbox" /></td>
			    	</tr>
<?
			}
?>
				  ...
				  <input type="button" value="" class="btnCancel" title="Cancel" onclick="parent.hideLightview();" />
                                ...
                                <input type="hidden" name="SamplingSize" value="<?=$iSize?>"/>
                  </form>
                  </td>
              </tr>
            </table>
