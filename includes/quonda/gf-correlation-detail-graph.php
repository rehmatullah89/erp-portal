<?
	...
	***********************************************************************************************
	...
	$sSQL = "SELECT dt.id, dt.type, COALESCE(SUM(gfd.defects), 0)
			 FROM tbl_qa_reports qa, tbl_gf_report_defects gfd, tbl_defect_codes dc, tbl_defect_types dt
			 $sConditions AND qa.id=gfd.audit_id AND gfd.code_id=dc.id AND dc.type_id=dt.id AND qa.report_id='6'
			 GROUP BY dc.type_id";
	$objDb->query($sSQL);
	...
	for ($i = 0; $i < count($sData['Id']); $i ++)
	{
		...
		{
			$sSQL = "SELECT COALESCE(SUM(gfd.defects), 0)
					 FROM tbl_qa_reports qa, tbl_gf_report_defects gfd, tbl_defect_codes dc, tbl_defect_types dt
					 $sConditions AND qa.id=gfd.audit_id AND gfd.code_id=dc.id AND dc.type_id=dt.id AND qa.report_id='6' AND dc.type_id='{$iTypes[$j]}' AND qa.user_id='{$sData['Id'][$i]}'
					 GROUP BY dc.type_id";
			$objDb->query($sSQL);
	...
	$objTitle = $objChart->addTitle("Auditors Correlation (Greige Fabric)", "verdana.ttf", 17);
	$objTitle->setPos(0,0);
	...
	$sChart = $objChart->makeSession("CorrelationDetailDf");
?>
