<?
	...
	***********************************************************************************************
	...
	$iTotalDefects = 0;
        ...
        $sSQL = "Select SUM(if(qrd.nature = '1',qrd.defects,0)) as Major, SUM(if(qrd.nature = '0',qrd.defects,0)) as Minor, qrd.area_id, dc.type_id, dc.code, dc.defect from tbl_qa_report_defects qrd, tbl_defect_codes dc where dc.id=qrd.code_id AND qrd.audit_id='$Id' Group By dc.id, qrd.area_id Order By dc.type_id,dc.code";
        $objDb->query($sSQL);
        $iCount = $objDb->getCount( );
        $iPreviousTypeId = 0;
        $newLine = 3.6;
	...
	if (count($sDefects) > 0)
	{
		...
		for ($i = 0; $i < $iPages; $i ++, $iCurrentPage ++)
		{
			...
			for ($j = 0; $j < 4 && $iIndex < count($sDefects); $j ++, $iIndex ++)
			{
				...
				$sSQL = "SELECT IF($sDefectQuery='', defect, $sDefectQuery) as _Defect,
								(SELECT IF($sTypeQuery IS NULL, type, $sTypeQuery) FROM tbl_defect_types WHERE id=dc.type_id) AS _Type
						 FROM tbl_defect_codes dc
						 WHERE code='$sDefectCode' AND report_id='$iReportId'";
                                
				$objDb->query($sSQL);

				$sDefect = $objDb->getField(0, "_Defect");
				...
				$sInfo .= ("Area: ".getDbValue("IF($sAreaQuery IS NULL, area, $sAreaQuery)", "tbl_defect_areas", "id='$sAreaCode'")."\n");
