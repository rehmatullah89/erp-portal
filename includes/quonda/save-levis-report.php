	...
	***********************************************************************************************
	...
	if ($bFlag == true)
	{
		...
		for ($i = 0; $i < $iCount; $i ++)
		{
			...
			if ($DefectId > 0)
			{
				$sSQL  = "UPDATE tbl_qa_report_defects SET code_id='$Code', defects='$Defects', area_id='$Area', nature='$Nature', sample_no='$SampleNo', remarks='$Remarks', status='$Status', date_time=NOW() WHERE id='$DefectId'";
				$bFlag = $objDb->execute($sSQL);
			}
			...
			else
			{
				...
				$sSQL  = ("INSERT INTO tbl_qa_report_defects (id, audit_id, code_id, defects, area_id, nature, sample_no, remarks, picture, status, date_time) VALUES ('$DefectId', '$Id', '$Code', '$Defects', '$Area', '$Nature', '$SampleNo', '$Remarks', '{$DefectId}-{$Picture}', '$Status', NOW())");
				$bFlag = $objDb->execute($sSQL);
			}
