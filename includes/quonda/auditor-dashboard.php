<?
	...
	***********************************************************************************************
	\*********************************************************************************************/
?>
				  <h2><?= $sAuditorsList[$Auditor] ?> Audits (<?= formatDate($FromDate) ?> to <?= formatDate($ToDate) ?>)</h2>
...
<?
	...
	$sSQL = "SELECT DISTINCT(audit_date)
	         FROM tbl_qa_reports
	         WHERE (audit_date BETWEEN '$FromDate' AND '$ToDate') AND
	               (user_id='$Auditor' OR (group_id>'0' AND group_id IN (SELECT id FROM tbl_auditor_groups WHERE FIND_IN_SET('$Auditor', users))))
	               $sConditions
...
?>
						  <tr>
							<td align="right"><?= formatDate($sDate) ?></td>
...
<?
		$iPrevious = 0;
		$sConditions  = " WHERE audit_date='$sDate' AND (user_id='$Auditor' OR (group_id>'0' AND group_id IN (SELECT id FROM tbl_auditor_groups WHERE FIND_IN_SET('$Auditor', users))))";
		$sConditions .= " AND FIND_IN_SET(report_id, '$sReportTypes') AND FIND_IN_SET(audit_stage, '$sAuditStages') ";
...
//						(SELECT line FROM tbl_lines WHERE id=tbl_qa_reports.line_id) AS _Line,
		$sSQL = "SELECT id, '0' AS location_id, '' AS details, 'A' AS _Type, master_id, TIME_TO_SEC(start_time) AS _StartTime, TIME_TO_SEC(end_time) AS _EndTime, audit_date, audit_code, vendor_id, brand_id, style_id, po_id,
						start_time, end_time, report_id, audit_stage, audit_result, custom_sample, total_gmts, approved, status, checked_gmts, created_at,
		...
		for ($j = 0; $j < $iCount; $j ++)
		{
			...
			$iMajorDefects   += $objDb->getField($j, '_GfDefects');
			$iDefectPics      = $objDb->getField($j, '_DefectPics');
			$iDefectPics     += $objDb->getField($j, '_GfDefectPics');
...
<?
				...
				if ($iReportId == 14 || $iReportId == 34)
				{
					...
					$sAuditorType  = getDbValue("auditor_type", "tbl_users", "id='$Auditor'");
