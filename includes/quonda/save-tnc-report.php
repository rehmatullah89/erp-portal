	...
	***********************************************************************************************
	...
	if ($bFlag == true)
	{
		...
		for ($i = 0; $i < $iCount; $i ++)
		{
			...
			$Result      = IO::strValue("Result".$i);
                        $Picture  = IO::getFileName($_FILES["Picture".$i]['name']);


			if ($DefectId > 0)
				$sSQL  = "UPDATE tbl_tnc_report_defects SET lot_no='$LotNo', roll_no='$RollNo', width='$Width', ticket_meters='$TicketMeters', actual_meters='$ActualMeters', holes='$Holes', slubs='$Slubs', stains='$Stains', fly='$Fly', other='$Other', result='$Result', date_time=NOW() WHERE id='$DefectId'";

			else
			{
                                $DefectId = getNextId("tbl_tnc_report_defects");
				
				$sSQL  = ("INSERT INTO tbl_tnc_report_defects (id, audit_id, lot_no, roll_no, width, ticket_meters, actual_meters, holes, slubs, stains, fly, other, result, date_time) VALUES ('$DefectId', '$Id', '$LotNo', '$RollNo', '$Width', '$TicketMeters', '$ActualMeters', '$Holes', '$Slubs', '$Stains', '$Fly', '$Other', '$Result', NOW())");
			}
                            ...
                            if ($_FILES["Picture{$i}"]['name'] != "" && $LotNo > 0 && $RollNo > 0)
                            {
                                    ...
                                    if(strtolower($sExtension) == '.jpg' || strtolower($sExtension) == '.jpeg')
                                    {
                                        ...
                                        if (@move_uploaded_file($_FILES["Picture{$i}"]['tmp_name'], ($sBaseDir.TEMP_DIR.$sDefectPic)))
                                        {
                                                ...
                                                $PrevImage = getDbValue("picture", "tbl_tnc_report_defects", "id='$DefectId'");

                                                if(!empty($PrevImage) && @file_exists($sQuondaDir.$PrevImage))
                                                    @unlink($sQuondaDir.$PrevImage);

                                                $sSQL  = "UPDATE tbl_tnc_report_defects SET picture='$sDefectPic' WHERE id='$DefectId'";
                                                $bFlag = $objDb->execute($sSQL);
                                        }                                               
                                    }
                            }
		...
		if ($bFlag == true && $sDefectIds != "")
		{
                        ...
                        $sQuondaDir = ($sBaseDir.QUONDA_PICS_DIR.$sYear."/".$sMonth."/".$sDay."/");
                        $DefectPictures = getList("tbl_tnc_report_defects", "id", "picture", "id NOT IN ($sDefectIds) AND audit_id='$Id' AND picture!=''");
			...
			$sSQL  = "DELETE FROM tbl_tnc_report_defects WHERE id NOT IN ($sDefectIds) AND audit_id='$Id'";
                        $bFlag = $objDb->execute($sSQL);
		}
