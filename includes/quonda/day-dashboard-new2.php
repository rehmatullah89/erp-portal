...
<?
	$sConditions  = " WHERE qa.audit_date='$FromDate' AND FIND_IN_SET(qa.report_id, '$sReportTypes') AND NOT FIND_IN_SET(qa.report_id, '$sQmipReports') ";
	...
	{
		if ($Brand > 0 && $Vendor > 0)
			$sConditions .= " AND po.customer LIKE '$Customer' AND po.vendor_id='$Vendor' AND po.brand_id='$Brand' ";

		else if ($Brand > 0)
			$sConditions .= " AND po.customer LIKE '$Customer' AND po.brand_id='$Brand' AND FIND_IN_SET(po.vendor_id, '{$_SESSION['Vendors']}') ";

		else if ($Vendor > 0)
			$sConditions .= " AND po.customer LIKE '$Customer' AND FIND_IN_SET(po.brand_id, '{$_SESSION['Brands']}') AND po.vendor_id='$Vendor' ";

		else
			$sConditions .= " AND po.customer LIKE '$Customer' AND FIND_IN_SET(po.brand_id, '{$_SESSION['Brands']}') AND FIND_IN_SET(po.vendor_id, '{$_SESSION['Vendors']}') ";
	}

	
	$sStyleCategoriesSql = str_replace("category_id", "s.category_id", $sStyleCategoriesSql);
	$sStyleCategoriesSql = str_replace("sub_season_id", "s.sub_season_id", $sStyleCategoriesSql);
	$sStyleCategoriesSql = str_replace("program_id", "s.program_id", $sStyleCategoriesSql);
	$sStyleCategoriesSql = str_replace("design_no", "s.design_no", $sStyleCategoriesSql);
	$sStyleCategoriesSql = str_replace("design_name", "s.design_name", $sStyleCategoriesSql);
	

	if ($Brand > 0)
		$sConditions .= " AND s.sub_brand_id='$Brand' $sStyleCategoriesSql";

	else
	{
		if ($_SESSION['UserType'] != 'MATRIX' && $_SESSION['UserType'] != 'TRIPLETREE')
			$sConditions .= " AND FIND_IN_SET(s.sub_brand_id, '{$_SESSION['Brands']}') $sStyleCategoriesSql";

		else
			$sConditions .= " AND (qa.style_id='0' OR (FIND_IN_SET(s.sub_brand_id, '{$_SESSION['Brands']}') $sStyleCategoriesSql)";
	}

	if ($Region > 0)
		$sConditions .= " AND v.country_id='$Region' ";
...
/*
	...
	if ($Brand == 0 && $Region == 162 && getDbValue("show_user_schedules", "tbl_users", "id='{$_SESSION['UserId']}'") == "Y" || $_SESSION["Admin"] == "Y")
	{
		$iLocations = array( );
	...
	if (($_SESSION['UserType'] == 'MATRIX' || $_SESSION['UserType'] == 'TRIPLETREE') && $Region == 162)
	{	
		$iAuditors = array( );
...
<?
		if (@strpos($sCity, "Activities") !== FALSE && ($_SESSION['UserType'] == 'MATRIX' || $_SESSION['UserType'] == 'TRIPLETREE') && $Region == 162)
		{		
			$sSQL = "SELECT id,
					 ...
					 WHERE date='$FromDate' AND FIND_IN_SET(user_id, '$sActivityAuditors')
					       AND user_id IN (SELECT id FROM tbl_users WHERE country_id='$sCityLocations')
