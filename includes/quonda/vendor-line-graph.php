<?
	...
	***********************************************************************************************
	...
	$fTargetDhu  = getDbValue($sStageDr, "tbl_vendors", "id='$iVendor'");
	$sStageTitle = getDbValue("stage", "tbl_audit_stages", "code='$AuditStage'");
	...
	$sSQL = "SELECT qa.line_id,
			 ...
			 WHERE po.id=qa.po_id AND qa.audit_type='B' AND qa.audit_result!='' AND qa.report_id!='6' AND qa.audit_stage='$AuditStage' $sAuditorSQL
	...
	$iIndex = @array_search($Line, $iLines);

	$sLineName    = $sLines[$iIndex];
	$fLineDhu     = $fDhu[$iIndex];
	$iLineGmts    = $objDb->getField($iIndex, "_Gmts");
	$iLineDefects = $objDb->getField($iIndex, "_Defects");
?>
						<div id="VendorLinesChart">loading...</div>

						<script type="text/javascript">
						<!--
						var objChart = new FusionCharts("scripts/fusion-charts/charts/Column3D.swf", "VendorLines", "100%", "340", "0", "1");
...
<?
	...
	{
?>
											"<set tooltext='Line: <?= str_replace("\n", "", $sLines[$i]) ?>{br}DHU: <?= $fDhu[$i] ?>%' label='<?= str_replace("\n", "", $sLines[$i]) ?>' value='<?= $fDhu[$i] ?>' link='<?= ("{$_SERVER['PHP_SELF']}?OrderNo={$OrderNo}&StyleNo={$StyleNo}&AuditCode={$AuditCode}&Brand={$Brand}&Vendor={$Vendor}&FromDate={$FromDate}&ToDate={$ToDate}&AuditStage={$AuditStage}&Line={$iLines[$i]}&Step=2") ?>' />" +
<?
	}
?>
