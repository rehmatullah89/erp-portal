<?
	...
	***********************************************************************************************
	...
	if (strtotime($sAuditDate) < strtotime("2013-08-01"))
	{
	...
	{
		if ($Brand == 111)
			$iPageCount = $objPdf->setSourceFile($sBaseDir."templates/converse-knits-old.pdf");

		else
			$iPageCount = $objPdf->setSourceFile($sBaseDir."templates/nike-knits.pdf");
	}

	else
	{
		if ($Brand == 111)
			$iPageCount = $objPdf->setSourceFile($sBaseDir."templates/converse-knits.pdf");
	...
	$objPdf->Text(185, 45.1, formatNumber($iShipQty, false));

	$objPdf->Text(24, 52.3, $sDestination);
	$objPdf->Text(79, 52.3, $sSizeTitles);
	$objPdf->Text(195, 52.3, formatNumber($fCartonsShipped, false));

	$objPdf->SetXY(78, 49.8);
	$objPdf->MultiCell(22, 3.2, $sSizeTitles, 0);

	$objPdf->SetXY(116, 49.8);
	$objPdf->MultiCell(57, 3.2, $sColors, 0);


	$objPdf->Text(26, 60.2, $sDescription);
	$objPdf->Text(140, 61.7, $sAuditStatus);
	$objPdf->Text(175, 61.7, strtoupper($sAuditStage));
	...
	$objPdf->Text(33, 79.5, $sStartTime);
	$objPdf->Text(74, 79.5, $sEndTime);
	$objPdf->Text(112, 79.5, formatNumber($fAql, true, (($fAql < 1) ? 2 : 1)));

	$objPdf->SetFont('Arial', 'B', 7);
	$objPdf->Text(177, 79.5, (($sAuditResult == "P") ? "PASS" : (($sAuditResult == "F") ? "FAIL" : "HOLD")));
	...
	$objPdf->Text(188, 225.5, $iTotalMajors);
	$objPdf->Text(199, 225.5, $iTotalMinors);
	...
	$objPdf->Text(195, 234, @round((($iTotalMajors / $iTotalGmts) * 100), 2));
