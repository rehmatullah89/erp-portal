<?
	...
	***********************************************************************************************
	...
	$objPdf->SetFont('Arial', '', 8);
        ...
        {
            $objPdf->SetFont('Arial', '', 6);
            $objPdf->SetXY(59, 44.5);
            $objPdf->MultiCell(40, 2.8, $sAllPos, 0);
            $objPdf->SetFont('Arial', '', 8);
        }
        else
            $objPdf->Text(60, 48.3, ($sAllPos));
        
        if(strlen($sParent) > 25)
        {
            $objPdf->SetFont('Arial', '', 7);
            $objPdf->SetXY(58, 50.5);
            $objPdf->MultiCell(40, 2.8, mb_convert_encoding($sParent, 'ISO-8859-1', 'UTF-8'), 0);
            $objPdf->SetFont('Arial', '', 8);
        }
        else
            $objPdf->Text(60, 54.5, mb_convert_encoding($sParent, 'ISO-8859-1', 'UTF-8'));
        ...
        if(strlen($sDescription) > 45)
        {
            ...
            $objPdf->SetXY(138, 44.5);
            $objPdf->MultiCell(60, 2.8, $sDescription, 0);
            $objPdf->SetFont('Arial', '', 8);
        }else
        ...
        if ($sLatitude != "" && $sLongitude != "")
	{
		...
		$objPdf->Write(5, "(". formatNumber($sLatitude, true, 8).",". formatNumber($sLongitude, true, 8).")", "http://maps.google.com/maps?q={$sLatitude},{$sLongitude}&z=12");
                ...
                if($sMapPicture != "" && @file_exists($sBaseDir.'files/maps/'.$sMapPicture))
                    $objPdf->Image($sBaseDir."files/maps/".$sMapPicture, 68, 146, 120,68);
                else
                {
                    if($sFactoryLatitude != "" && $sFactoryLongitude != "")
                    ...
                    $objPdf->Image($sBaseDir."files/maps/googlemapImage-{$sAuditCode}.jpg", 68, 146, 120,68);
        ...
        {
                $objPdf->SetFont('Arial', '', 6);
                $objPdf->Text(99, 135, "Audit location/cordinates are un-available.");
		...
		$objPdf->SetTextColor(6, 82, 195);
            ...
            $objPdf->Text(99, 135, getDbValue("city", "tbl_vendors", "id='$iVendor'"));
	...
	$objPdf->MultiCell(70, 2.6, $sProductColourRemarks, 0);
        ...
        $objPdf->SetXY(130, 83);
	$objPdf->MultiCell(70, 2.6, str_replace("’","'",$sCartonStickerRemarks), 0);
	...
	if ($sWovenWeight != "")
		$objPdf->Image(($sBaseDir.'images/icons/tick.gif'), (($sWovenWeight == "P") ? 109 : 122), 159, 4);

        $objPdf->SetXY(130, 159);
	$objPdf->MultiCell(70, 2.6, $sWovenWeightRemarks, 0);
	...
	if ($sCartonNos != "")
	{
		...
		for ($i = 1, $iIndex = 0; $i <= 7 && $iIndex < count($sCartonNos); $i ++)
		{
			for ($j = 1; $j <= 6 && $iIndex < count($sCartonNos); $j ++, $iIndex ++)
				$objPdf->Text((($j * 31) - 17), (47 + ($i * 5)), $sCartonNos[$iIndex]);
		}

		$objPdf->Text(80, 87.5, count($sCartonNos));
	}
...
/*	$iTotalSizeQty   = 0;
        ...
        for($i = 0; $i < $iCount; $i ++)
	{
        ...
        $objPdf->Text(146, 232, $TotalCritical);
	$objPdf->Text(167, 232, $TotalMajor);
        $objPdf->Text(188, 232, $TotalMinor);
        
        $objPdf->Text(146, 237, "0");
	$objPdf->Text(167, 237, $iAqlChart[$iTotalGmts]["2.5"]);
	$objPdf->Text(188, 237, $iAqlChart[$iTotalGmts]["4"]);
	...
	$objPdf->Text(146, 242, (($sAuditResult == "P") ? "PASS" : "FAIL"));
...
//echo $sPoint."~~~>N:".$fNTolerance."~~~>P:".$fPTolerance."~~~>V:".$sSizeFindings["{$j}-{$iPoint}"]."~~~>PT:".$fPositiveTolerance."~~~>NT:".$fNegativeTolerance."~~~MV~~".$fMeaseuredValue."#SP#".$fSpecs."<br/>";                                                        
							$iOut ++;
						}
					}
