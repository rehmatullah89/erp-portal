<?
	...
	***********************************************************************************************
	...
	$iPageCount  = $objPdf->setSourceFile($sBaseDir."templates/vendor-roving-end.pdf");
	...
	for ($i = 0; $i < count($sCodes); $i ++)
	{
		...
		if ($iArea > 0)
		{
			...
			$objPdf->Text( (($i <= 33) ? 64 : 173), $fTop, getDbValue("area", "tbl_defect_areas", "id='$iArea'"));
		}

		if ($iDefects > 0)
		{
			$objPdf->SetFont('Arial', '', 7);
			$objPdf->Text( (($i <= 33) ? 91 : 200), $fTop, $iDefects);
		}
