<?
	...
	***********************************************************************************************
	...
	$iPageCount  = $objPdf->setSourceFile($sBaseDir."templates/vendor-final.pdf");
	...
	$iPageCount  = $objPdf->setSourceFile($sBaseDir."templates/vendor-final-comments.pdf");
	$iTemplateId = $objPdf->importPage(1, '/MediaBox');
	...
	for ($i = 64; $i < count($sCodes); $i ++)
	{
		...
		{
			$objPdf->SetFont('Arial', '', 6);
			$objPdf->Text( (($i <= 77) ? 64.5 : 173.5), $fTop, getDbValue("area", "tbl_defect_areas", "id='$iArea'"));
		}

		if ($iDefects > 0)
		{
			$objPdf->SetFont('Arial', '', 7);
			$objPdf->Text( (($i <= 77) ? 92 : 201), $fTop, $iDefects);
		}
	}
