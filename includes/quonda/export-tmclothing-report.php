<?
	...
	***********************************************************************************************
	...
	$iAqlLevel          = $objDb->getField(0, "aql_level");
	...
	$iPageCount  = $objPdf->setSourceFile($sBaseDir."templates/tm-clothing.pdf");
	...
	$objPdf->Text(38, 106, $sSizeTitles);

	
	$objPdf->Text(142, 64.7, $iAuditQuantity);
	$objPdf->Text(142, 68.5, $iTotalGmts);
	$objPdf->Text(187, 68.5, (($iCheckLevel == 1) ? ' First' : 'Second'));
	...
	for($i = 0; $i < $iCount; $i ++)
	{
		...
		{
			$objPdf->SetXY(33, ($iTop - 1.8));
			$objPdf->MultiCell(90, 2, $sDefect, 0);
		}
		
		$objPdf->Text(137, $iTop, $sDefectAreas);
	...
	$iMinorMajors = @floor($iTotalMinor / 3);
        
	$objPdf->Text(173, 237, $iTotalCritical);
	$objPdf->Text(185, 237, $iTotalMajor);
	$objPdf->Text(196, 237, (($iMinorMajors > 0) ? "{$iTotalMinor} ({$iMinorMajors})" : $iTotalMinor));
	

	@list($iMajors, $iMinors) = getAqlDefects($iTotalGmts, $fAql, $iReportId, 0,$iCheckLevel);

	$objPdf->Text(173, 241, '0');
	$objPdf->Text(185, 241, $iMajors);
	...
	else if (($iTotalMajor + $iMinorMajors) <= $iMajors)
	...
	else
	{
		...
		if (@isset($iThreshold[$iTotalGmts]))
		{
			if (($iTotalMajor + $iMinorMajors) < $iThreshold[$iTotalGmts])
			{
				$sWorkMenShipResult = "Flag for Second Check";
				$sAuditResult       = "F";
			}
			
			else
			{
				$sWorkMenShipResult = "Failed Beyond Second Check Threshold";
				$sAuditResult       = "F";
			}
		}
		
		if ($sWorkMenShipResult == "")
			$sWorkMenShipResult = "Fail";
	}
	
	
	$objPdf->Text(168, 246, $sWorkMenShipResult);
	...
	$objPdf->Text(142, 93, (($iMinorMajors > 0) ? "{$iTotalMinor} ({$iMinorMajors})" : $iTotalMinor));
