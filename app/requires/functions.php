<?
	...
	***********************************************************************************************
	...
	function getList($sTable, $sKey, $sValue, $sConditions = "", $sOrderBy = "", $sLimit = "")
	{
		...
		for ($i = 0; $i < $iCount; $i ++)
		{
			if ($sGuest == "Y" && $sTable == "tbl_users")
				$sList[$objDbGlobal->getField($i, 0)] = (($objDbGlobal->getField($i, 0) == 584) ? $objDbGlobal->getField($i, 1) : ("Brand - ".$objDbGlobal->getField($i, 0)));
			
			else if ($sGuest == "Y" && $sTable == "tbl_brands")
				$sList[$objDbGlobal->getField($i, 0)] = (($objDbGlobal->getField($i, 0) == 256) ? $objDbGlobal->getField($i, 1) : ("Brand - ".$objDbGlobal->getField($i, 0)));

			else if ($sGuest == "Y" && $sTable == "tbl_vendors")
	...
	function getDbValue($sField, $sTable, $sConditions, $sOrderBy = "", $sLimit = "")
	{
		...
		if ($sGuest == "Y" && @strpos($sField, "_id") === FALSE)
		{
