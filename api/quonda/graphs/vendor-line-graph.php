<?
	...
	***********************************************************************************************
	...
	$sSQL = "SELECT qa.line_id,
			 ...
			 WHERE po.id=qa.po_id AND qa.audit_type='B' AND qa.audit_result!='' AND qa.report_id!='6' AND qa.audit_stage='$AuditStage'
...
<?
	...
	{
?>
											"<set tooltext='Line: <?= $sLines[$i] ?>{br}DHU: <?= $fDhu[$i] ?>%' label='<?= $sLines[$i] ?>' value='<?= $fDhu[$i] ?>' link='<?= (SITE_URL."api/quonda/graphs-s2.php?User={$User}&OrderNo={$OrderNo}&AuditCode={$AuditCode}&Brand={$Brand}&Vendor={$Vendor}&FromDate={$FromDate}&ToDate={$ToDate}&AuditStage={$AuditStage}&Line={$iLines[$i]}") ?>' />" +
<?
	}
?>
