<?
	...
	***********************************************************************************************
	...
	else
	{
		...
		else if ($Offline == "" && ($Po == "" || !@in_array($AuditStage, $sAuditStages) ||
		         					!@in_array($AuditStatus, array("1st", "2nd", "3rd", "4th", "5th", "6th")) || $SampleSize == 0 || $Colors == "" ||
		         					!@in_array($AuditResult, array("F", "H", "P", "A", "B", "C"))))
		{
			$aResponse['Status'] = "ERROR";
		...
		else
		{
			...
			$iBrandId = (int)$objDb->getField(0, 2);
			...
			if ($bFlag == true)
			{
				...
				$iAuditMode = getDbValue("audit_mode", "tbl_qa_reports", "id='$iAuditCode'");

				if ($iAuditMode != 2)
					$iAuditMode = 1;


				$sSQL  = "UPDATE tbl_qa_reports SET po_id='$iPoId', additional_pos='$sAdditionalPos', style_id='$iStyleId', brand_id='$iBrandId', audit_stage='$AuditStage', audit_status='$AuditStatus', total_gmts='$SampleSize', colors='$Colors', sizes='$iSizes', audit_result='$AuditResult', ship_qty='$ShipQty', re_screen_qty='$ReScreenQty', total_cartons='$CartonsInspected', cartons_shipped='$CartonsShipped', carton_size='$CartonSize', max_defects='$iDefectsAllowed', approved_sample='$ApprovedSample', shipping_mark='$ShippingMark', packing_check='$PackingCheck', qa_comments='$Comments', date_time=NOW( ), audit_mode='$iAuditMode' WHERE id='$iAuditCode'";
				...
				else
				{
					if ($iReportId == 7)
					{
						...
						if ($sWorkingNo == ""&& $iStyleId > 0)
							$sWorkingNo = getDbValue("style", "tbl_styles", "id='$iStyleId'");

						else if ($sWorkingNo != "" && $iStyleId == 0 && $Style != "")
							$iStyleId = getDbValue("id", "tbl_styles", "brand_id IN (67,75) AND style LIKE '$Style'");



						$sSQL  = ("UPDATE tbl_qa_reports SET ship_qty='".IO::intValue("PiecesAvailable")."', beautiful_products='".IO::intValue("BeautifulProducts")."', cutting='".IO::intValue("Cutting")."', sewing='".IO::intValue("Sewing")."', finishing='".IO::intValue("Finishing")."', packing='".IO::intValue("Packing")."' WHERE id='$iAuditCode'");
						$bFlag = $objDb->execute($sSQL, true, $User, $sUser);

						if ($bFlag == true)
						{
							if (getDbValue("COUNT(*)", "tbl_ar_inspection_checklist", "audit_id='$iAuditCode'") == 0)
								$sSQL  = ("INSERT INTO tbl_ar_inspection_checklist (audit_id, model_name, working_no, fabric_approval, counter_sample_appr, garment_washing_test, color_shade, appearance, handfeel, printing, embridery, fibre_content, country_of_origin, care_instruction, size_key, adi_comp, colour_size_qty, polybag, hangtag, ocl_upc, carton_no_checked)
								                                            VALUES ('$iAuditCode', '".IO::strValue("ModelName")."', '".IO::strValue("WorkingNo")."', '".IO::strValue("FabricApproval")."', '".IO::strValue("CounterSampleAppr")."', '".IO::strValue("GarmentWashingTest")."', '".IO::strValue("ColorShade")."', '".IO::strValue("Appearance")."', '".IO::strValue("Handfeel")."', '".IO::strValue("Printing")."', '".IO::strValue("Embridery")."', '".IO::strValue("FibreContent")."', '".IO::strValue("CountryOfOrigin")."', '".IO::strValue("CareInstruction")."', '".IO::strValue("SizeKey")."', '".IO::strValue("AdiComp")."', '".IO::strValue("ColourSizeQty")."', '".IO::strValue("Polybag")."', '".IO::strValue("Hangtag")."', '".IO::strValue("OclUpc")."', '".IO::strValue("CartonsNoChecked")."')");

							else
								$sSQL  = ("UPDATE tbl_ar_inspection_checklist SET model_name='".IO::strValue("ModelName")."', working_no='$sWorkingNo', fabric_approval='".IO::strValue("FabricApproval")."', counter_sample_appr='".IO::strValue("CounterSampleAppr")."', garment_washing_test='".IO::strValue("GarmentWashingTest")."', color_shade='".IO::strValue("ColorShade")."', appearance='".IO::strValue("Appearance")."', handfeel='".IO::strValue("Handfeel")."', printing='".IO::strValue("Printing")."', embridery='".IO::strValue("Embridery")."', fibre_content='".IO::strValue("FibreContent")."', country_of_origin='".IO::strValue("CountryOfOrigin")."', care_instruction='".IO::strValue("CareInstruction")."', size_key='".IO::strValue("SizeKey")."', adi_comp='".IO::strValue("AdiComp")."', colour_size_qty='".IO::strValue("ColourSizeQty")."', polybag='".IO::strValue("Polybag")."', hangtag='".IO::strValue("Hangtag")."', ocl_upc='".IO::strValue("OclUpc")."', carton_no_checked='".IO::strValue("CartonsNoChecked")."' WHERE audit_id='$iAuditCode'");
					...
					{
						$sSQL  = ("UPDATE tbl_qa_reports SET defective_gmts='".IO::floatValue("GmtsDefective")."', batch_size='".IO::strValue("BatchSize")."', packed_percent='".IO::floatValue("PercentPacked")."', description='".IO::strValue("Description")."', ship_qty='".IO::intValue("BatchSize")."' WHERE id='$iAuditCode'");
						$bFlag = $objDb->execute($sSQL, true, $User, $sUser);
			...
			if ($bFlag == true)
			{
				...
				$iShipQty     = $ShipQty;
				$fDhu         = getDbValue("dhu", "tbl_qa_reports", "id='$iAuditCode'");
...
/*
						...
						$objEmail->addBCC("sanfernando@mgfsourcing.com", "Sanjaya Anuk Fernando");
						...
						$objEmail->addBCC("ukaviratne@mgfsourcing.com", "Udaya Kaviratne");
						$objEmail->addBCC("Balachandran@mgfsourcing.com", "Manoj Balachandran");
...
/*
			if ($bFlag == true && $iReportId != 6)
			{
				...
				$fDhu = @round((($iDefects / $SampleSize) * 100), 2);
...
*/
			...
			else
			{
				...
				$aResponse['Status'] = "ERROR";
				$aResponse["Error"]  = (($sMessage == "") ? "An ERROR Occured while processing your request." : "$sMessage ");
			}
		}
	}
...
/*
	...
	$objEmail->Body    = @json_encode($_REQUEST)."<br><br>".@json_encode($aResponse)."<br><br>".$sSQL."<br><br>".mysql_error();
