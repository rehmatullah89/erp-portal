<?
	...
	***********************************************************************************************
	...
	$sSQL = "SELECT id, name, designation_id, audits_manager, picture, status, vendors, brands FROM tbl_users WHERE username='$Username' AND password=PASSWORD('$Password')";
	...
	if ($objDb->query($sSQL) == true)
	{
		if ($objDb->getCount( ) == 1)
		{
			if ($objDb->getField(0, "status") == "A")
			{
				...
				$bAudits_Manager = $objDb->getField(0, "audits_manager");
				...
				$sAuditorMode  = getDbValue("count(*)", "tbl_qa_reports", "user_id='$iUserId'");
				...
				$sAuditorsList  = getList("tbl_users", "id", "name", "designation_id IN (Select id from tbl_designations where department_id IN (15,41)) AND status='A'");
...
//					$sDefectCodes[] = @implode("|-|", $sCodes);
				...
				$aResponse['Department_id'] = $iDepartment; //for auditors screen
				$aResponse['AuditorMode'] = $bAudits_Manager; //for auditors screen
				...
				$aResponse['Auditors']    = $sAuditorsList;
