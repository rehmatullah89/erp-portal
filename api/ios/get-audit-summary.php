...
*/
		...
		$sSQL = "SELECT id, user_id, vendor_id,brand_id, audit_code, po_id, style_id, audit_date, start_time, end_time, audit_stage, audit_result, total_gmts, report_id, dhu, colors,
			                 ...
			                 (SELECT tbl_brands.id FROM tbl_po,tbl_brands WHERE tbl_po.id=tbl_qa_reports.po_id and tbl_brands.id=tbl_po.brand_id) AS _BrandID,
			          ...
			          FROM tbl_qa_reports where audit_code ='$AuditCode'
...
//echo $iCount; exit(0);
			for ($i = 0; $i < $iCount; $i ++)
			{
				...
				$iBrand       = $objDb->getField($i, "_BrandID");
...
$sSQL = "SELECT qa.audit_code, qa.audit_date, dc.code
											 ...
											 WHERE qa.id=qad.audit_id AND qad.code_id=dc.id  AND IF(qa.report_id=10, qad.nature='1', TRUE) AND qa.audit_code ='$AuditCode'  $sConditions

											 UNION

							SELECT qa.audit_code, qa.audit_date, dc.code
											 FROM tbl_qa_reports qa, tbl_gf_report_defects gfd, tbl_defect_codes dc
											 WHERE qa.id=gfd.audit_id AND gfd.code_id=dc.id AND qa.report_id='6' AND qa.audit_code ='$AuditCode' $sConditions
...
//echo $sSQL; exit;
											 ...
											 $iCount2 = $objDb2->getCount( );
												...
												for ($j = 0; $j < $iCount2; $j++)
												{
													...
													$sAudit_Date = $objDb2->getField($j, "audit_date");
													//print $sAudit_Date.":";
													$sAudit_Code = $objDb2->getField($j, "audit_code");
								...
								//print $sCode;
															...
															$files = preg_grep( "/_{$sCode}_/", $sAuditPictures );
																...
																//$defectImages[$iDefectType][] = $sDefectPic;
																	$defectImages[] = $sDefectPic;
																}

												}
				//print_r($defectImages); exit;
				...
				$sAuditStage = $sAuditStage==""?"N/A":$sAuditStage;
				$sAuditResult = $sAuditResult==""?"N/A":$sAuditResult;
...
//				$dDataObject["style_image_url"] = $sDefectPic;
				...
				$dDataObject["defect_images_url"] = $defectImages;
