<?
	...
	***********************************************************************************************
	...
	{
		$sSQL = "SELECT id, name, email, status, vendors, brands, style_categories, report_types, audit_stages, app_skip_image, user_type FROM tbl_users WHERE MD5(id)='$User'";
		$objDb->query($sSQL);
		...
		else
		{
			...
			$sSQL = "SELECT 'A' AS _Type, audit_code, IF(ISNULL(device_id), '', device_id) AS _DeviceId, vendor_id, '0' AS location_id, user_id, report_id, style_id, po_id, additional_pos, sizes, colors, 
			                start_time, end_time, audit_stage, audit_result, aql, dhu, audit_quantity, total_gmts, checked_gmts, qa_comments, '' AS details, start_date_time, TIME_TO_SEC(start_time) AS _StartTime, published
					 FROM tbl_qa_reports
					 $sConditions AND audit_date='$Date'

					 UNION

					 SELECT 'S' AS _Type, CONCAT('T', LPAD(id, 5, '0')) AS audit_code, '' AS _DeviceId, '0' AS vendor_id, location_id, '0' AS user_id, '0' AS report_id, '0' AS style_id, '0' AS po_id, '' AS additional_pos, '' AS sizes, '' AS colors, 
					        start_time, end_time, '' AS audit_stage, '' AS audit_result, '0' AS aql, '0' AS dhu, '0' AS audit_quantity, '0' AS total_gmts, '0' AS checked_gmts, '' AS qa_comments, details, '' AS start_date_time, TIME_TO_SEC(start_time) AS _StartTime, '' AS published
...
//				if ($sUserType == "MGF")
				...
				{
					$iSizes = @explode(",", $iSizes);

					foreach ($iSizes as $iSize)
					{
						$iSamplingSize = getDbValue("id", "tbl_sampling_sizes", "size LIKE '{$sSizes[$iSize]}'");
						$sSizeDetails  = array( );
				...
				$sAuditView = array("Auditor"        => md5($iAuditorId),
									...
									"AuditStage"     => $sAuditStagesList[$sAuditStage],
									"Aql"            => $fAql,
				...
				$sAudits[] = array("Type"           => $sType,
								   ...
								   "AuditStageText" => $sAuditStagesList[$sAuditStage],
				                   "AuditResult"    => $sAuditResult,
								   "ReportId"       => $iReportId,
				                   "StyleId"        => $iStyleId,
				                   "SampleSize"     => $iSampleSize,
								   "Completed"      => (($sAuditResult != "" && $sComments != "" && ($iSampleChecked == 0 || ($iSampleChecked > 0 && $iSampleSize > 0 && $iSampleChecked == $iSampleSize))) ? "Y" : "N"),
				                   "OnGoing"        => $sOnGoing,
				                   "Color"          => $sColor,
				                   "Block"          => $iBlock,
				                   "Location"       => $sLocation,
			...
			$sSQL = "SELECT audit_date, DAYOFMONTH(audit_date) AS _Day, audit_stage, COUNT(*) AS _Audits
			         FROM tbl_qa_reports
			         $sConditions AND (audit_date BETWEEN DATE_SUB(CURDATE( ), INTERVAL 14 DAY) AND DATE_SUB(CURDATE( ), INTERVAL 1 DAY))
			         GROUP BY audit_date, audit_stage

			         UNION

			         SELECT from_date AS audit_date, DAYOFMONTH(from_date) AS _Day, 'Non-Prod.' AS audit_stage, COUNT(*) AS _Audits
					 FROM tbl_user_schedule
					 WHERE user_id='$iUser' AND (from_date BETWEEN DATE_SUB(CURDATE( ), INTERVAL 14 DAY) AND DATE_SUB(CURDATE( ), INTERVAL 1 DAY))
			         GROUP BY from_date
			...
			$sSQL = "SELECT audit_date, DAYOFMONTH(audit_date) AS _Day, audit_stage, COUNT(*) AS _Audits
			         FROM tbl_qa_reports
			         $sConditions AND (audit_date BETWEEN DATE_ADD(CURDATE( ), INTERVAL 1 DAY) AND DATE_ADD(CURDATE( ), INTERVAL 14 DAY))
			         GROUP BY audit_date, audit_stage
