<?
	...
	***********************************************************************************************
	...
	require_once 'PHPExcel/RichText.php';
        ...
        /* FTP Connection */ 
        $sftp = new Net_SFTP('125.209.75.188', 22);
	...
	$iRow ++;
               ...
               $sSQL = "SELECT qr.*,
                       ...
                       (Select brand from tbl_brands where id=qr.brand_id) as _Brand,
                       (Select etd_required from tbl_po_colors where po_id=p.id AND etd_required != '0000-00-00' Order By etd_required Limit 0,1) as _ETD_DATE,
	...
	for ($i = 0; $i < $iCount; $i ++)
	{
		...
		$objPHPExcel->getActiveSheet()->setCellValue("F{$iRow}", ($sAuditorType == 4?"MCA":"FCA"));
		...
		$objPHPExcel->getActiveSheet()->setCellValue("I{$iRow}", ($sAuditDate == '0000-00-00'?'0000-00-00':date('Y-m-d',strtotime('-10 hour',strtotime($sAuditDate)))));
		$objPHPExcel->getActiveSheet()->setCellValue("J{$iRow}", ($sStartTime == '00:00:00'?'00:00:00':date('H:i:s',strtotime('-10 hour',strtotime($sStartTime)))));
                $objPHPExcel->getActiveSheet()->setCellValue("K{$iRow}", ($sEndTime == '00:00:00'?'00:00:00':date('H:i:s',strtotime('-10 hour',strtotime($sEndTime)))));
                $objPHPExcel->getActiveSheet()->setCellValue("L{$iRow}", ($sFinalAuditDate == '0000-00-00'?'0000-00-00':date('Y-m-d',strtotime('-10 hour',strtotime($sFinalAuditDate)))));
                $objPHPExcel->getActiveSheet()->setCellValue("M{$iRow}", $iStyle);
                ...
                $objPHPExcel->getActiveSheet()->setCellValue("T{$iRow}", ($sETD_DATE == '0000-00-00'?'0000-00-00':date('Y-m-d',strtotime('-10 hour',strtotime($sETD_DATE)))));
                ...
                $objPHPExcel->getActiveSheet()->setCellValue("BH{$iRow}", ($sCreatedAt == ''?'':date('Y-m-d H:i:s',strtotime('-10 hour',strtotime($sCreatedAt)))));
                $objPHPExcel->getActiveSheet()->setCellValue("BI{$iRow}", ($sModifiedAt == ''?'':date('Y-m-d H:i:s',strtotime('-10 hour',strtotime($sModifiedAt)))));
                $objPHPExcel->getActiveSheet()->setCellValue("BJ{$iRow}", ($iCreatedBy == 0?$sAuditor:$sAuditorsList[$iCreatedBy]));
                $objPHPExcel->getActiveSheet()->setCellValue("BK{$iRow}", ($sModifiedAt != ''?($iModifiedBy != 0?$sAuditorsList[$iModifiedBy]:$sAuditor):''));
