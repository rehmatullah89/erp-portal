<?
	...
	***********************************************************************************************
	...
	if(@in_array($_SESSION["UserType"], array("JCREW")))
	{
		...
		$Po             = (int)@$OrderNo[0];                
		$AdditionalPOs  = implode(",", array_diff($OrderNo, array(@$OrderNo[0])));
	}
	...
	if ($_SESSION['Flag'] == "")
	{
		...
		if ($bFlag == true)
		{
                    $sSQL  = ("INSERT INTO tbl_qa_reports (user_id, group_id, department_id, audit_type_id, vendor_id, brand_id, unit_id, po_id, style_id, master_id, colors, sizes, total_gmts, report_id, line_id, audit_stage, audit_date, start_time, end_time, approved, custom_sample, commission_type, date_time, created_at, status, additional_pos, published, cutting_lot_no, audit_quantity, inspection_level, aql, check_level, app_version)
										   VALUES ('$Auditor', '$Group', '$Department', '$AuditType', '$Vendor', '$iBrand', '$Unit', '$Po', '$StyleNo', '$iMasterId', '".@implode(",", IO::getArray("Colors"))."', '".@implode(",", IO::getArray("Sizes"))."', '$TotalGmts', '$Report', '".IO::intValue("Line")."', '".IO::strValue("AuditStage")."', '$AuditDate', '$StartTime', '$EndTime', 'Y', '$sCustomSample', '$sInspectionType', NOW( ), NOW( ), '', '$AdditionalPOs', '$sPublished', '$LotNo', '$OfferedQty', '$InspectionLevel', '$AqlLevel', '$CheckLevel', '276')");
			$bFlag = $objDb->execute($sSQL);
		}
