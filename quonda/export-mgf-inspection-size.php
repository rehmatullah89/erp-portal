<?
	...
	***********************************************************************************************
	...
	$sAuditorsList      = getList("tbl_users", "id", "name");
        ...
        $sExcelFile = ($sBaseDir.TEMP_DIR."INSPECTION_AUDIT_SIZE.xlsx");
	...
	$objPHPExcel->getProperties()->setTitle("INSPECTION_AUDIT_SIZE");
	$objPHPExcel->getProperties()->setSubject("");
	$objPHPExcel->getProperties()->setDescription("INSPECTION_AUDIT_SIZE");
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B'.$iRow, "Size");
	...
	$sSQL = "SELECT qr.id, qr.sizes, qr.user_id, qr.audit_code, qr.created_at
	...
	for ($i = 0; $i < $iCount; $i ++)
	{
		...
		$iAuditor       = $objDb->getField($i, "user_id");
                ...
                $sSizes         = getDbValue("GROUP_CONCAT(size SEPARATOR ', ')", "tbl_sizes", "id IN ({$objDb->getField($i, "sizes")})") ;
                $sCreatedAt     = $objDb->getField($i, "created_at");
                
                $iSizes = explode(",", $sSizes);
                
                foreach($iSizes as $sSize)
                {
                    $objPHPExcel->getActiveSheet()->setCellValue("A{$iRow}", $iReportId);
                    $objPHPExcel->getActiveSheet()->setCellValue("B{$iRow}", $sSize);
	...
	// Rename sheet
	$objPHPExcel->getActiveSheet()->setTitle('INSPECTION_AUDIT_SIZE');
	...
	$objWriter->save($sExcelFile);
        ...
        $sFileName = 'INSPECTION_AUDIT_SIZE.xlsx';
