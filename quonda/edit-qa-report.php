<?
	...
	***********************************************************************************************
	...
	$Id      = ((IO::intValue('Id') > 0) ? IO::intValue('Id') : intval(str_replace("S", "", IO::strValue('AuditCode'))));
	...
	if ($objDb->getCount( ) == 1)
	{
		...
		$WorkmanshipResult     = $objDb->getField(0, "workmanship_result");
	...
	if ($AdditionalPos != "")
        ...
        $sSelectedStyles = $Style;
                
        if($AdditionalStyles != "")
            $sSelectedStyles .= ",{$AdditionalStyles}";
...
?>
  <script type="text/javascript" src="scripts/quonda/edit-qa-report.js.php?Id=<?= $Id ?>&ReportId=<?= $ReportId ?>&AuditDate=<?= $AuditDate ?>&Colors=<?=$Colors?>&SampleSize=<?= $TotalGmts ?>"></script>
					...
					$("#QaDefects .defectRecords").each(function(iIndex)
					{
						...
						if(FIR != '26')
						{
							...
							$(this).find("select.defectArea").attr("id", ("Area" + iIndex));
                                                        ...
                                                        if(FIR >= '53')
                                                        {
                                                            $(this).find("select.defectLot").attr("name", ("LotNo" + iIndex));
                                                            $(this).find("select.defectLot").attr("id", ("LotNo" + iIndex));
                                                        }
...
<?
		...
		//else if ($ReportId == 54)
		//	@include($sBaseDir."includes/quonda/edit-new-tnc-report.php");
                
                else if ($ReportId == 47 || $ReportId >= 53)
                    @include($sBaseDir."includes/quonda/edit-general-report.php");
